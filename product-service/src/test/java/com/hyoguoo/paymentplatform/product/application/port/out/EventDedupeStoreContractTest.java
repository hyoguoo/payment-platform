...
/**
 * EventDedupeStore 포트 — deleteExpired 시그니처 컴파일 계약 테스트.
 *
 * <p>실제 삭제 동작은 JdbcEventDedupeStoreCleanupTest(D-2)에서 검증한다.
 * 이 테스트는 포트 인터페이스에 deleteExpired(Instant, int) 시그니처가 정확히 존재함을
 * 컴파일 수준에서 강제한다 — 시그니처 드리프트 시 RED.
 */
@DisplayName("EventDedupeStore 포트 deleteExpired 시그니처 계약")
