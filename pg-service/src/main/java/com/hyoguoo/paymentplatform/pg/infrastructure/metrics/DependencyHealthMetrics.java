 ...
 * </ul>
 *
 * <p>DB 조회는 {@code metrics.pg.dependency.timeout-seconds} 타임아웃 가드로 감싸
 * Hikari connectionTimeout(30s) 등 블로킹 경로로 폴러가 직렬화되는 것을 방지한다.
 * 타임아웃 시 0(DOWN)으로 처리된다.
 *
...
public class DependencyHealthMetrics {
     ...
     *
     * <p>DB 조회는 타임아웃 가드가 적용되며, 조회 완료 후
     * {@code dependency_health_last_poll_timestamp_seconds} 를 갱신한다.
     * 타임아웃·조회 실패 모두 게이지를 0(DOWN) 으로 기록한다.
     */
