 ...
 *
 * <p>{@link com.hyoguoo.paymentplatform.pg.application.service.PgDlqService#handle}
 * 에서 {@code transitToQuarantined} 가 true(non-terminal → QUARANTINED CAS 성공)를
 * 반환할 때 호출된다. terminal CAS 는 1회만 통과하므로 같은 orderId 의 중복 DLQ 진입은
 * 이 카운터를 증가시키지 않는다 — 멱등.
 *
