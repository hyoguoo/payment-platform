...
import com.hyoguoo.paymentplatform.pg.presentation.dto.PgVendorStatusResponse;
 ...
 * payment-service 의 Feign client가 관리자 상세 화면 렌더 시 호출한다.
 *
 * <ul>
 *   <li>GET /api/v1/confirmations/{orderId}/attempts — 확정 시도 이력 조회</li>
 *   <li>GET /api/v1/confirmations/{orderId}/vendor-status — 벤더 상태 조회(격리 종결 판단용)</li>
 * </ul>
 *
 ...
 * 다만 벤더 상태 조회는 조회 서비스 자신이 예외를 확인 불가로 흡수하므로, 이 컨트롤러까지 예외가
 * 올라오지 않는다.
 */
@RestController
...
public class PgAttemptHistoryController {
    ...
    private final PgVendorStatusQueryService pgVendorStatusQueryService;
    ...
    @GetMapping("/{orderId}/vendor-status")
    public ResponseEntity<PgVendorStatusResponse> getVendorStatus(@PathVariable String orderId) {
        PgVendorStatusView view = pgVendorStatusQueryService.getVendorStatus(orderId);
        return ResponseEntity.ok(PgVendorStatusResponse.from(orderId, view));
    }
}
