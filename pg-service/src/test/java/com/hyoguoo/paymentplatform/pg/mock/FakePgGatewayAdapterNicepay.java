...
/**
 * NICEPAY 전용 PgStatusLookupPort + PgConfirmPort Fake.
 * K14: selector 단위 테스트에서 NICEPAY 전략 구별용으로 사용한다.
 *
 * <p>FakePgGatewayAdapter 와의 차이: {@link #supports(PgVendorType)} 이 NICEPAY 만 true 반환.
 */
public class FakePgGatewayAdapterNicepay implements PgStatusLookupPort, PgConfirmPort {
    ...
    public PgConfirmResult confirm(PgConfirmRequest request) {
        ...
        if (result == null) {
            throw new IllegalStateException(
                    "FakePgGatewayAdapterNicepay: confirm 결과가 설정되지 않음 — orderId=" + request.orderId());
        }
        return result;
    }
    ...
    public PgStatusResult getStatusByOrderId(String orderId) {
        ...
        if (result == null) {
            throw new IllegalStateException(
                    "FakePgGatewayAdapterNicepay: status 결과가 설정되지 않음 — orderId=" + orderId);
        }
        return result;
    }
