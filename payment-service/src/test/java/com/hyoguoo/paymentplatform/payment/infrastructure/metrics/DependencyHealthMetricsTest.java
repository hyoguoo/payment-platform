...
class DependencyHealthMetricsTest {
    ...
    private RedisConnectionFactory mockDedupeFactory;
    private RedisConnectionFactory mockStockFactory;
    ...
    void setUp() throws Exception {
        ...
        mockDedupeFactory = mock(RedisConnectionFactory.class);
        mockStockFactory = mock(RedisConnectionFactory.class);

        setupHealthyDb();
        setupHealthyRedisDedupe();
        setupHealthyRedisStock();

        metrics = new DependencyHealthMetrics(
                meterRegistry, mockDataSource, mockDedupeFactory, mockStockFactory, FIXED_CLOCK, 2L);
    }
    ...
    @Test
    @DisplayName("health UP 상태면 게이지 1 — DB·redis-dedupe·redis-stock 모두 정상이면 각 게이지 1.0")
    void health_UP_상태면_게이지1() {
        ...
        assertThat(componentGaugeValue(DependencyHealthMetrics.COMPONENT_REDIS_DEDUPE)).isEqualTo(1.0);
        assertThat(componentGaugeValue(DependencyHealthMetrics.COMPONENT_REDIS_STOCK)).isEqualTo(1.0);
    }
    ...
    void health조회_타임아웃이면_게이지0() throws Exception {
        ...
        DependencyHealthMetrics timeoutMetrics = new DependencyHealthMetrics(
                timeoutRegistry, blockingDataSource, mockDedupeFactory, mockStockFactory,
                FIXED_CLOCK, 0L);
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // Test 5: redis-dedupe / redis-stock 컴포넌트 분리
    // ──────────────────────────────────────────────────────────────────────────

    @Test
    @DisplayName("redis-dedupe 와 redis-stock 컴포넌트 분리 — 한쪽 DOWN 이 다른쪽 게이지에 영향 없음")
    void redis_dedupe와_redis_stock_컴포넌트_분리() {
        // given: redis-dedupe 는 연결 실패, redis-stock 은 정상
        when(mockDedupeFactory.getConnection()).thenThrow(
        ...
        // then: redis-dedupe 게이지 0, redis-stock 게이지 1
        assertThat(componentGaugeValue(DependencyHealthMetrics.COMPONENT_REDIS_DEDUPE)).isEqualTo(0.0);
        assertThat(componentGaugeValue(DependencyHealthMetrics.COMPONENT_REDIS_STOCK)).isEqualTo(1.0);
    }
    ...
    private void setupHealthyRedisDedupe() {
        RedisConnection mockConn = mock(RedisConnection.class);
        when(mockDedupeFactory.getConnection()).thenReturn(mockConn);
        when(mockConn.ping()).thenReturn("PONG");
    }

    private void setupHealthyRedisStock() {
        RedisConnection mockConn = mock(RedisConnection.class);
        when(mockStockFactory.getConnection()).thenReturn(mockConn);
        when(mockConn.ping()).thenReturn("PONG");
    }
