 ...
 *
 * <p>Instant 전환 — clockDateTimeProvider() 가 {@link Instant} 를 반환하는 계약을 검증한다.
 * <ol>
 ...
 *       {@code isInstanceOf(Instant.class)} 단정이 실패한다 — Instant 전환 회귀를 잡는다.
...
class JpaConfigClockDateTimeProviderTest {
    ...
    /**
     * Instant 전환 가드 — clockDateTimeProvider() 가 고정 Clock 의 Instant 를 그대로 반환한다.
     *
     * <p>고정 instant 기준으로 동일 Instant 가 반환되는지 직접 단정한다.
     */
    @Test
    @DisplayName("단위 가드 — clockDateTimeProvider() 가 Clock 고정 instant 를 반환한다")
    ...
    /**
     * 단위 가드 — KST Zone Clock 을 주입해도 반환값은 동일 Instant 이어야 한다.
     *
     * <p>Zone 과 무관하게 동일 instant 를 반환한다는 계약을 검증한다.
     */
    @Test
    @DisplayName("단위 가드 — KST Zone Clock 을 주입해도 반환값은 동일 Instant 다")
    ...
    /**
     * 단위 가드 — clockDateTimeProvider() 와 Spring 기본 CurrentDateTimeProvider 는 고정 Clock 상황에서 다른 값을 낸다.
     *
     * <p>이 테스트는 두 provider 가 서로 구분 가능함을 단정한다.
     * 고정 과거 Clock 을 주입하면 현재 시각을 반환하는 기본 provider 와 달라진다.
     */
    @Test
    @DisplayName("설계 단정 — clockDateTimeProvider 는 고정 Clock 을 반영하므로 기본 CurrentDateTimeProvider 와 구별된다")
