...
/**
 * PgFeignClient 전용 Feign 설정 — payment-service 가 pg-service 를 HTTP 로 부르는 최초 경로의 오류 매핑.
 *
 * <p>ErrorDecoder 가 4xx/5xx → 도메인 예외 매핑.
 * PgFeignClient 호출에서 발생하는 4xx/5xx 응답을 도메인 예외로 매핑한다.
 *
 * <ul>
 *   <li>404 → {@link PgAttemptHistoryNotFoundException} (PG_ATTEMPT_HISTORY_NOT_FOUND)</li>
 *   <li>429 / 502 / 503 / 504 → {@link PgAttemptHistoryServiceRetryableException} (PG_SERVICE_UNAVAILABLE)</li>
 ...
 * 전역 {@code @Configuration} 으로 등록되면 다른 FeignClient(product-service/user-service) 에도
 * 영향을 미친다.
 */
public class PgFeignConfig {

    private static final Logger log = LoggerFactory.getLogger(PgFeignConfig.class);
    ...
    private RuntimeException mapToException(Response response) {
        ...
        if (status == HttpStatus.NOT_FOUND.value()) {
            LogFmt.warn(log, LogDomain.PAYMENT_GATEWAY, EventType.PG_SERVICE_NOT_FOUND,
                    () -> "status=" + status + " body=" + body);
            return PgAttemptHistoryNotFoundException.of(PaymentErrorCode.PG_ATTEMPT_HISTORY_NOT_FOUND);
        }
        if (status == HttpStatus.TOO_MANY_REQUESTS.value()
            ...
            LogFmt.warn(log, LogDomain.PAYMENT_GATEWAY, EventType.PG_SERVICE_RETRYABLE,
                    () -> "status=" + status);
            return PgAttemptHistoryServiceRetryableException.of(PaymentErrorCode.PG_SERVICE_UNAVAILABLE);
        }
        LogFmt.warn(log, LogDomain.PAYMENT_GATEWAY, EventType.PG_SERVICE_UNEXPECTED,
                () -> "status=" + status + " body=" + body);
        return new IllegalStateException(
                "pg-service 오류 status=" + status + " body=" + body
        );
    }
