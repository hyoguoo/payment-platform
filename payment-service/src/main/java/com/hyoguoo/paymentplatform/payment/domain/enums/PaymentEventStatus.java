...
public enum PaymentEventStatus {
     ...
     * AWAITING_RESULT는 리컨실러가 되돌린 뒤 확정 결과를 여전히 기다리는 상태이므로 non-terminal.
     * 이 메서드는 LOCAL_TERMINAL_STATUSES Set 중복 선언을 대체하는 SSOT 판별자다.
     */
    public boolean isTerminal() {
        return switch (this) {
            case DONE, FAILED, CANCELED, PARTIAL_CANCELED, EXPIRED -> true;
            case READY, IN_PROGRESS, AWAITING_RESULT, QUARANTINED -> false;
        };
    }
     ...
     *
     * <p>READY, IN_PROGRESS, AWAITING_RESULT 세 상태에서만 confirm 결과를 적용할 수 있다.
