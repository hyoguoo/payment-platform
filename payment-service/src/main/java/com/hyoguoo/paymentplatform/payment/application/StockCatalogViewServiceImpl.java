 ...
 *
 * <p>try 범위는 {@link ProductCatalogQueryPort#getPage} 호출과 그 결과를 담는
 * {@link ProductCatalogLookupResult#available} 조립까지다 — 뷰 변환은 이 서비스가 하지
 * 않으므로, product-service 조회 실패와 payment-service 자체 매핑 버그가 로그에서 섞이지
 * 않는다.
 */
@Slf4j
@Service
...
public class StockCatalogViewServiceImpl implements StockCatalogViewService {
    ...
    public ProductCatalogLookupResult getPage(int page, int size) {
        try {
            return ProductCatalogLookupResult.available(productCatalogQueryPort.getPage(page, size));
