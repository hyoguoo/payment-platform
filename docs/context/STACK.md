...
**스케줄러 역할별 목록**:
- payment-service: `DedupeCleanupWorker`(payment_event_dedupe 만료 행 청소) / `PaymentScheduler`(READY 만료) / `PaymentReconciler`(IN_PROGRESS 정체 복원) / `OutboxWorker`(outbox PENDING 폴링·Kafka 발행, `@Scheduled(fixedDelayString = "${scheduler.outbox-worker.fixed-delay-ms:5000}")`)
