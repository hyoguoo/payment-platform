...
스키마 위치가 두 패턴 — **payment/pg 는 `db/migration/`**(단일, seed 없음), **product/user 는 `db/schema/` + `db/seed/`** 분리. `docker` 프로필의 seed 취급은 서비스마다 다르다 — **product 는 차단, user 는 포함**(빼면 사용자 행 0 으로 뜬다). 상세: [`stack/flyway-operations.md`](stack/flyway-operations.md).
...
**스케줄러 역할별 목록**:
- payment-service: `DedupeCleanupWorker`(payment_event_dedupe 만료 행 청소) / `PaymentScheduler`(READY 만료) / `PaymentReconciler`(IN_PROGRESS 정체분을 결과 대기로 되돌리고, 결과 대기 2차 임계 초과분을 격리로 — `reconciler.in-flight-timeout-seconds` 기본 300, `reconciler.awaiting-result-timeout-seconds` 기본 900) / `OutboxWorker`(outbox PENDING 폴링·Kafka 발행) / `PaymentStateMetrics`(결제 상태별 카운트 gauge 갱신, `@ConditionalOnProperty` 없이 빈 등록되나 `@EnableScheduling`이 SchedulerConfig 게이트 안에 있어 `scheduler.enabled=true` 필요) / `PaymentHealthMetrics`(stuck IN_PROGRESS·결과 대기 적체·max-retry 이상 탐지 gauge 갱신, 동일 게이트) / `PaymentOutboxMetrics`(payment_outbox PENDING 지표 갱신, `@ConditionalOnProperty(scheduler.enabled=true)` 직접 부착) / `DependencyHealthMetrics`(의존성 가용성 폴링 게이지 `dependency_up{component}`, availability 알람 그룹 소비, 동일 게이트)
...
하나는 종류가 비어 있다. 종류가 빈 쪽은 누가 끝났는지 알 수 없어 위 필터를 그냥 통과하므로 따로
건너뛴다. 코드를 쓴 서브에이전트는 종류를 싣고 오고, 메인 스레드는 `Stop` 이 덮는다.
