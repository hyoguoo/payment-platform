...
#### [OUTBOX-STALE-PUBLISH-AFTER-AWAITING-RESULT] — 결과를 기다리는 결제에 확정 명령이 뒤늦게 나가는 경로

- **현황**: 브로커 장애로 발행이 밀리면 결제는 리컨실러가 결과 대기(`AWAITING_RESULT`)로 되돌린 채 결과를 기다린다. 그 사이 `payment_outbox` 행은 독립적으로 재시도를 계속하고, 브로커가 복구되면 확정 명령을 발행한다. 벤더가 승인하면 되돌릴 수 없는 과금이 남고 payment 측은 종결 상태로 판정해 조용히 넘긴다.
- **가드 상태**: `OutboxRelayService.relay` 에 발행 직전 결제 상태 가드가 있으나 결과 대기는 확정 결과 적용 가능 상태라 **그 창에서는 발동하지 않는다**. 다만 CONFIRM-RESULT-NONRETRYABLE-STATUS 가 2차 임계(기본 900초) 초과 시 격리로 올리므로, 격리에 도달한 뒤에는 가드가 실제로 걸린다 — 무기한이던 창에 상한이 생겼다.
...
- **처방**: 재동기화 실행 전 해당 상품에 미종결 선차감이 있는지 확인하는 가드.
- **STOCK-GATE-PER-PRODUCT 이후 쉬워졌다**: 선차감 기록(`stock_hold_record`)이 상품별로 미회수 차감을 들고 있으므로, 격리 주문을 새로 조회할 필요 없이 해당 상품의 잡음 상태 행 존재만 보면 된다. 그때까지는 운영 절차로 막는다.
...
#### [FAKE-REPO-FIND-ALIASING] — 테스트용 저장소의 조회가 내부 참조를 그대로 돌려준다

- **현황**: `FakePaymentEventRepository` 의 `save` 는 방어적 복사를 하지만 `find*` 계열은 저장소가 들고 있는 객체를 그대로 반환한다. 위임 메서드는 인자를 도메인 메서드로 먼저 바꾼 뒤 조건부 갱신을 부르므로, 조회로 얻은 객체를 그대로 넘기면 선행조건 검사가 이미 바뀐 값을 보고 정상 케이스에도 충돌로 오판한다.
- **영향**: 프로덕션은 SQL 조건절로만 판정해 영향이 없다. 다만 실제 리컨실러와 실제 위임 메서드를 이 Fake 와 조립하는 **첫 테스트가 조용히 항상 실패**한다. 지금은 그 조합의 테스트가 없어 드러나지 않는다.
- **처방**: 조회도 방어적 복사로 통일한다. 그때 자식 주문 리스트의 깊은 복사까지 함께 봐야 한다 — 현재 복사는 리스트를 얕게 넘긴다.
- **출처**: CONFIRM-RESULT-NONRETRYABLE-STATUS ship 리뷰(domain-expert minor, 재리뷰에서 후속 등재 권고).
...
- **처방**: 키가 상품 기준 해시태그라 **같은 노드에 떨어진 상품들은 한 스크립트로 묶을 수 있다.** 그러면 왕복이 상품 수가 아니라 노드 수에 비례해, 상품 10개짜리 주문도 노드 3대면 최대 3회다.
- **판단**: 지금은 상품마다 한 번씩 부르는 단순한 형태다. 미리 넣으면 원자성이 그룹 안에서만 성립하는 구조가 더해져 복잡도만 오른다 — 왕복이 실제로 병목으로 드러나면 도입한다.
...
### [GUARDRAIL-HOOK-SELFTEST] — 가드레일 훅 자체를 검증하는 수단이 없다

- **현황**: `.claude/hooks/` 의 두 스크립트는 다른 모든 변경을 검증하는 위치에 있으면서 정작 자신은 어떤 자동 검사도 받지 않는다. 도입 시 위반 파일을 만들어 수동으로 확인했을 뿐이다.
- **영향**: `jq`·`python3`·`shasum` 부재, 경로 공백, git 명령 실패 같은 엣지에서 조용히 통과(`exit 0`)하도록 짜여 있어, 훅이 무력화돼도 아무도 모른다. 검증 장치가 꺼져 있는 것과 켜져 있는 것을 구분할 방법이 없다.
- **처방**: 픽스처 파일에 대해 두 스크립트의 종료 코드를 확인하는 셸 테스트. 저장소에 셸 테스트 기반이 없으므로 도입 방식(별도 스크립트 / Gradle 태스크 편입 / CI step)부터 정해야 한다.
- **출처**: PR #153 ship 리뷰 minor.

### [IDEMPOTENCY-KEY-GOLDEN-TEST] — 멱등키 회귀 가드가 값을 고정하지 않는다

- **현황**: `IdempotencyKeyHasherTest` 의 네 테스트가 전부 "같은 입력이면 같은 해시 / 순서 무관 / 다른 입력이면 다른 해시" 식 상대 비교다. 고정 입력에 대한 기대값을 박아 두지 않았다.
- **영향**: 해시 산출에 필드가 하나 섞여 들어가도 테스트는 전부 통과한다. 배포 중 신·구 인스턴스가 같은 요청에 다른 키를 내면 그 창에서 한 결제가 두 주문으로 갈라진다.
- **처방**: 고정 입력에 대한 리터럴 기대 해시 한 건 추가. 알고리즘을 의도적으로 바꿀 때만 그 값을 함께 고치게 된다.
- **출처**: PR #153 ship 도메인 리뷰 minor. 이 PR 의 패키지 이동은 해시 산출에 클래스·패키지명을 쓰지 않아 값이 바뀌지 않음을 소스 대조로 확인했다.
