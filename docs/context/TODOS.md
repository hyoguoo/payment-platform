...
#### [GUARD-SKIP-EAGER-REGISTER] — PaymentConfirmGuardSkipMetrics lazy register 정렬

`PaymentConfirmGuardSkipMetrics:44-47` 의 Counter 가 가드 분기 내 lazy 등록. 이름/태그셋 충돌 시 `IllegalArgumentException` → not-retryable → DLQ 이론 경로 잔존. 신규 `PaymentEventFlowMetrics` 가 생성자 eager 등록 패턴을 채택했으므로 같은 모듈 내 비대칭. false 6종 status 카운터를 생성자에서 eager 등록으로 정렬해 이론 위험 제거. (OBSERVABILITY-COMPLETION review-2 domain-minor-3, 이연 수용)

**관련 코드**:
- `payment-service/.../core/common/metrics/PaymentConfirmGuardSkipMetrics.java:44-47`
