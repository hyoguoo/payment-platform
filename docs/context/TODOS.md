...
#### [AGENT-DOCS-STATIC-ANALYSIS] — `var`·`@Data`·null 반환·`catch (Exception)`·try 재할당 정적 강제 도입 (3/5 완료, SIGNAL-AND-GUARDRAIL-SWEEP Task 11)

- **현황**: 다섯 규칙 모두 `docs/context/conventions/code-style.md` 에 명문화돼 있었으나 자동 강제 수단이 없었다. `SIGNAL-AND-GUARDRAIL-SWEEP` Task 11 에서 문자열 패턴으로 판정 가능한 3규칙을 `config/checkstyle/checkstyle.xml` 에 `severity=warning`(빌드 미차단)으로 추가했다 — 기존 위반은 `config/checkstyle/checkstyle-suppressions.xml` 에 전량 억제해 기준선을 0 으로 만들었다.
...
- **잔여**: `catch (Exception)` swallow 금지, try 블록 내 외부 변수 재할당 금지 — 문자열 패턴만으로 판정 불가(구조 분석 필요), `SIGNAL-AND-GUARDRAIL-SWEEP` Task 12 로 넘어감(ArchUnit 도입 여부는 Task 12에서 결정).
- **처방**: Task 12 완료 시 이 항목 전체 종결.
