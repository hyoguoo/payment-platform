...
## fabricated approvedAt 마커 — SYNTHETIC_APPROVED_AT (Domain F-7 이관)

**배경:**
FCG(Final Confirmation Gate) 경로 및 DuplicateApprovalHandler 의 DB-absent APPROVED 경로에서
approvedAt 은 실제 벤더 응답이 아닌 Clock fallback(`OffsetDateTime.now(clock)`) 으로 채워진다.
현재 `ConfirmedEventPayload` / `ConfirmedEventMessage` 양쪽에 이 사실이 표기되지 않아
consumer(`PaymentConfirmResultUseCase`) 가 실 승인 시각과 합성 시각을 구분할 수 없다.

**제안:**
옵션 A: `ConfirmedEventPayload` / `ConfirmedEventMessage` 에 `boolean approvedAtSynthetic` 필드 추가.
        producer 쪽(PgFinalConfirmationGate/DuplicateApprovalHandler.buildApprovedPayload)이 `true` 설정.
        consumer 쪽이 해당 값을 읽어 로그·알림 분기 가능.
옵션 B: `reasonCode` 에 "SYNTHETIC_APPROVED_AT" 부착.

**영향 파일:**
- `pg-service/.../messaging/event/ConfirmedEventPayload.java`
- `payment-service/.../messaging/consumer/dto/ConfirmedEventMessage.java`
- `pg-service/PgFinalConfirmationGate.buildApprovedPayload`
- `pg-service/DuplicateApprovalHandler.buildApprovedPayload`

**제안 시점:**
- Phase 4 완료 후 별도 리팩토링 태스크. 현재 파급 범위(record cascading)가 커서 K8 범위 밖.

---

## primitive obsession — Money/OrderId/PaymentKey Value Object (Domain F-11 이관)

**배경:**
`PaymentEvent` / `PaymentOrder` 등 여러 도메인 객체에서 `BigDecimal amount`, `String orderId`,
`String paymentKey` 등이 원시 타입으로 처리된다. 이 타입들은 비즈니스 불변식(scale=0, 길이 제한 등)을
각 사용처에서 개별 방어해야 하므로 중복이 발생한다.

**제안:**
- `Money` value object: 원화 최소 단위 정수(scale=0) + non-negative 불변식 내장.
- `OrderId` value object: UUID 형식 검증 + factory 메서드.
- `PaymentKey` value object: 벤더 지정 포맷 검증.
- `AmountConverter`(pg-service) 는 payment-service 에 복제 도입 — `Money.toLong()` / `Money.fromLong()`.

**영향 범위:**
도메인 객체 시그니처 대규모 변경 + JPA 컨버터 + API 경계 직렬화 변경. 별도 토픽 설계 필수.

**제안 시점:**
- Phase 4 완료 후 별도 토픽 `DOMAIN-VALUE-OBJECTS` 로 설계·구현.
