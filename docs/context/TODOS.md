...
#### [AGENT-DOCS-STATIC-ANALYSIS] — `var`·`@Data`·null 반환·광범위 예외 삼킴·try 재할당 정적 강제 도입 (5/5 완료, SIGNAL-AND-GUARDRAIL-SWEEP Task 11+12)

- **현황**: 다섯 규칙 모두 `docs/context/conventions/code-style.md` 에 명문화돼 있었으나 자동 강제 수단이 없었다. `SIGNAL-AND-GUARDRAIL-SWEEP` Task 11 에서 문자열 패턴으로 판정 가능한 3규칙을, Task 12 에서 구조 판정이 필요한 나머지 2규칙을 각각 `config/checkstyle/checkstyle.xml` 에 `severity=warning`(빌드 미차단)으로 추가했다 — 기존 위반은 `config/checkstyle/checkstyle-suppressions.xml` 에 전량 억제해 기준선을 0 으로 만들었다.
  - `var` 키워드 금지(`VarKeywordUsage`): 기존 위반 **2건** 억제(둘 다 테스트 코드 — `AsyncConfigContextPropagationTest.java:60`, `PaymentTransactionCoordinatorTest.java:191`).
  - `@Data` 금지(`DataAnnotationUsage`): 기존 위반 **0건**(억제 항목 없음).
  - 공개 유스케이스·포트 null 반환 금지(`PublicUseCasePortNullReturn`): 기존 위반 **0건** — 적용 범위를 `application/usecase`, `application/port` 디렉토리로 좁히는 스코프 억제 1건 별도(위반 억제가 아니라 검사 대상 범위 한정용).
  - 광범위 예외(Exception/RuntimeException/Throwable/Error) 삼킴 금지(`SwallowedBroadException`, Task 12): 완전히 빈 catch 블록만 구조적으로 판정 — 기존 위반 **1건** 억제(`PgConfirmListenerSplitIntegrationTest.java:275`, 테스트 코드). 나머지 2건(`PaymentCheckoutConcurrencyIntegrationTest.java:76,141`)은 `presentation/` 디렉토리 블랑켓 억제에 이미 포함돼 별도 항목 불필요. 로그만 남기고 재throw/return 없이 흐름을 이어가는 더 약한 형태의 삼킴은 문자열/AST 어느 쪽으로도 재throw·fallback 여부까지 정확히 구분하기 어려워 이번 검사 대상 밖 — 실제 코드 전수 검토로 그런 사례가 현재 0건임을 수동 확인했다.
  - try 블록 내 외부 변수 재할당 금지(`TryBlockExternalReassignment`, Task 12): 기존 위반 **3건** 억제(`StockCatalogViewServiceImpl.java:36`, `PgAttemptHistoryViewServiceImpl.java:36` — 프로덕션 코드, `DuplicateApprovalHandlerListenerTest.java:73` — 테스트). 검사 범위는 try 문과 같은 블록에서 선언된 변수로 한정(중첩 블록 밖 변수는 추적 안 함).
- **구현 수단(Task 12 결정)**: ArchUnit 대신 checkstyle 커스텀 TreeWalker Check(`config/checkstyle/custom-checks/`) 를 선택했다 — ArchUnit 은 바이트코드 기반 클래스/메서드 의존 그래프(호출·필드 접근·애노테이션·계층)만 다뤄 "catch 블록 안에 재throw/return 문이 있는가", "변수가 try 밖에서 선언돼 try 안에서 재할당됐는가" 같은 메서드 본문 내부 statement 구조·제어 흐름을 표현할 수 없다. checkstyle 은 이미 프로젝트가 쓰는 도구이고 TreeWalker 가 완전한 AST 를 제공해 두 규칙 모두 자연스럽게 구현됐다. 새 외부 의존 없음 — `com.puppycrawl.tools:checkstyle:10.17.0`(checkstyle 플러그인이 이미 쓰는 것과 동일 버전)을 커스텀 Check 컴파일용으로 root project 전용 sourceSet 에 추가했을 뿐이다.
- **처방**: 완료. 신규 위반은 두 checkstyle 경고(`SwallowedBroadException`, `TryBlockExternalReassignment`) 가 PR 리뷰 시점에 드러낸다.
...
#### [CODE-READY-HANDLEUNKNOWNFAILURE-STALE] — convention 섹션의 낡은 메서드 참조 (완료, SIGNAL-AND-GUARDRAIL-SWEEP Task 14)

- **현황**: `.claude/skills/_shared/checklists/code-ready.md` convention 섹션의 "`catch (Exception e)` 없음 (있다면 `handleUnknownFailure` 경유)" 항목이 가리키던 `handleUnknownFailure` 메서드는 코드베이스 전체에 존재하지 않았다 — `outbox-only-refactor` 리팩터로 `PaymentFailureUseCase.handleUnknownFailure()` 가 삭제됐고(grep 결과 프로덕션 코드 0건, archive 문서에만 잔존), `AGENT-CONTEXT-OVERHAUL` Task 7 실행 중 발견됐다.
- **처방**: 완료. 항목을 현재 `error-logging.md` 규칙("`catch (Exception e)` swallow 금지 — 잡으면 LogFmt로 기록 후 재throw 또는 명시적 fallback")으로 교체했다. `scripts/check-agent-docs.py` 재실행으로 이 항목 관련 판정에 문제 없음을 확인.
