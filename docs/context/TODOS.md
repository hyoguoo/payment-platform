...
`PgInboxAmountService` 는 main 코드 호출처가 0인 dead service 다.
PCS-9 에서 포트 메서드(transitNoneToInProgress) 삭제에 따른 컴파일 에러 해소만 진행했으며,
dead service 자체 제거는 사용자 확인 후 별 토픽에서 처리한다.

본 토픽(PG-CONFIRM-LISTENER-SPLIT) 범위 외라고 명시 — m1 review finding 등록.
