...
#### [PRODUCT-TIME-ABSTRACTION] — product-service 시간 추상화 부재

product-service 에 `LocalDateTimeProvider` / `Clock` 같은 시간 추상화가 없어 `DedupeCleanupWorker` / `StockCommitConsumer` 가 `Instant.now()` 를 직접 호출. 서비스 전역 시간 추상화 도입 검토 (테스트 결정성 향상). payment-service 의 `LocalDateTimeProvider` 포트 패턴 또는 `Clock` 빈 주입 후보. TC-8 (시간 추상화 통합) 과 결이 같으므로 같이 처리 가치.

**관련 코드**:
- `product-service/.../infrastructure/scheduler/DedupeCleanupWorker.java`
- `product-service/.../infrastructure/messaging/consumer/StockCommitConsumer.java` (경로 확인 필요)
...
#### [SPOTBUGS-TEST-DEBT] — payment-service `spotbugsTest` 사전 부채

`./gradlew :payment-service:spotbugsTest` 가 `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` 4건 + `EI_EXPOSE_REP2` 1건으로 실패해 `./gradlew build` 전체가 BUILD FAILED 한다. EOS-FOLLOWUP-CLEANUP verify 중 발견됐으나 위반 클래스가 전부 이번 토픽 diff 밖이고 `main` 워크트리에서 동일 재현되는 **기존 부채**라 해당 토픽 범위에서 제외했다. `test` 태스크만으로는 노출되지 않아 그간 누락돼 있었다 — spotbugs 억제(`@SuppressFBWarnings`) 또는 테스트 코드 정정으로 build 게이트 회복 필요.

**관련 코드**:
- `payment-service/.../infrastructure/cache/StockCacheRedisAdapterTest.java`
- `payment-service/.../infrastructure/cache/StockCompensationAtomicLuaTest.java` / `StockDecrementAtomicLuaTest.java`
- `payment-service/.../integration/StockCompensationRecoveryIntegrationTest.java`
- `payment-service/.../mock/FakeMessagePublisher.java`
