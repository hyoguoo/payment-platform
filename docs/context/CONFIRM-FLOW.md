...
// L7 cascade 인지: DLQ + Reconciler 되돌리기 + 새 confirm 사이클 → 재고 발산 가능 (PG 멱등성으로 일반 차단)
```
...
| payment event IN_PROGRESS 장기 체류 | `PaymentReconciler` (`@Scheduled fixedDelayMs=120000, 2분`) — 1차 `findInProgressOlderThan` → 결과 대기(`AWAITING_RESULT`), 2차 `findAwaitingResultOlderThan` → 격리. 되돌리기 자체가 재처리를 부르지는 않는다(`OutboxWorker` 는 `payment_outbox` 기준) — 결과 대기는 늦게 온 확정 결과를 받을 수 있는 자리다 |
