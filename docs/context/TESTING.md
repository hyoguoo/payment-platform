...
**통합테스트 합산**: `jacocoTestReport` 가 `test` exec 에 더해 `integrationTest` exec 를 조건부 합산한다 (`afterEvaluate` + `tasks.findByName('integrationTest')` 가드 — payment/pg/product 보유, user/gateway/eureka 부재 시 자연 스킵). 합산 없이는 통합 경로로만 커버되는 usecase 분기가 미달로 잡혀 게이트가 거짓 실패한다.

**커버리지 게이트**: `jacocoTestCoverageVerification` 에 LINE `minimum` 을 둔다(element=`BUNDLE` — 테스트 없는 관리 클래스가 전체를 차단하지 않도록 번들 합산). 값은 통합테스트 합산 실측 - 안전마진으로 서비스별 편차 수용 (ext `jacoco.lineCoverageMinimum`: payment 0.89 / pg 0.91 / product 0.40 / user 0.0). baseline 은 현재 수치 동결이라 "결제 시나리오 누락을 게이트가 잡는다"는 의미는 아니다. user-service 는 측정 대상이 거의 없어 0.0(사실상 무게이트, 후속 과제).
