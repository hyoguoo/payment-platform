...
- **내부 작업·결정 ID 금지**: `D7`, `PET-8`, `SCR-8`, `DR-3`, `PCS-9`, `CBA-9`, `§6` 같은 토픽/태스크/결정 식별자 + 설계 옵션 라벨(`Option A/B`, `E1`)을 주석·Javadoc·로그 문자열·`@DisplayName` 에 박지 않는다. 작업이 끝나면 의미를 잃고 노이즈만 남는다. 코드만 읽는 사람이 docs 를 열어야 이해되면 실패다. 그 ID 가 가리키던 **의미**(예: `D7` → "종결 가드", `DR-1` → "멀티상품 멱등키 결정성")를 자연어로 풀어 쓴다.
