...
- 멱등 응답("이미 처리됨"): `PgGatewayDuplicateHandledException` → `DuplicateApprovalHandler` 가 vendor getStatus 재조회로 확정. 금액 대조가 먼저이고, 일치하면 접수 기록의 종결 여부로 갈라진다 — 종결이면 보관 결과 재발행, 종결 전이면 조회 상태가 승인일 때만 조회 결과로 APPROVED 종결(승인 시각은 `PgStatusResult.approvedAtRaw` 원문 사용), 승인 미확인이면 상태를 바꾸지 않고 물러난다
