...
> 최종 갱신: 2026-04-27 (MSA-TRANSITION + PRE-PHASE-4-HARDENING 봉인 + 코드 사실 재검증)
...
flowchart TD
    ...
    AC --> AC1{"EventDedupeStore.markWithLease<br/>eventUuid, leaseTtl=PT5M<br/>SET NX EX (two-phase lease)"}
    AC1 -->|false 이미 처리 중| AC1a["no-op"]
    AC1 -->|true 처리 권한 획득| AC2["paymentEvent 조회"]
    AC2 --> AD{"message.status"}
    AD -->|APPROVED| AE1["event.done approvedAt<br/>각 PaymentOrder별로 stock_outbox INSERT<br/>→ StockOutboxImmediateEventHandler AFTER_COMMIT<br/>→ payment.events.stock-committed 발행<br/>→ product-service 재고 확정"]
    AD -->|FAILED| AE2["markPaymentAsFail reasonCode<br/>각 PaymentOrder 별 redis-stock INCR 보상<br/>(product RDB 변경 없음 — 애초에 차감 X)"]
    AD -->|QUARANTINED| AE3["compensateStockCache (redis INCR)<br/>+ QuarantineCompensationHandler.handle<br/>수동 조사 알림"]
...
| 8 | pg | Kafka consume (`attempt` 헤더 파싱, 기본 1) | pg_inbox NONE→IN_PROGRESS CAS (amount 기록) |
| 9 | pg | Toss/Nicepay/Fake HTTP 호출 | APPROVED / FAILED (4xx) / retryable (5xx→self-loop) / DLQ (attempt≥4→QUARANTINED) |
| 10 | pg | pg_outbox 저장 → PgOutboxImmediateWorker (채널 + VT) relay | **Kafka `payment.events.confirmed` 발행** |
| 11 | payment | Kafka consume (two-phase lease markWithLease PT5M) | APPROVED → event DONE + stock_outbox INSERT → AFTER_COMMIT → `payment.events.stock-committed` / FAILED·QUARANTINED → event 전이 + redis-stock INCR 보상 |
...
- **PG 5xx/timeout**: pg-service 자체 retry (attempt < 4 self-loop) → 한도 초과 시 DLQ → pg_inbox QUARANTINED → payment `QuarantineCompensationHandler`. FCG (`PgFinalConfirmationGate`) 는 현재 호출처 0 — 미연결 (후속 Phase 예정)
...
- **중복 메시지 (payment 측)**: `EventDedupeStore` two-phase lease (markWithLease PT5M → extendLease P8D) + outbox IN_FLIGHT CAS. 실패 → remove → false → `payment.events.confirmed.dlq`
