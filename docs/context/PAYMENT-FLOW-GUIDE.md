...
> 최초 작성 2026-06-22 · 승급 2026-06-23 · 정정 2026-07-03(outbox 발행 실패 회복 경로 사실 정정) · 정정 2026-07-06(단계 명칭 개칭 + 문체 정리) · 갱신 2026-07-11(§C 회복 색인에 격리 관리자 안전 실패 종결·DLQ 수동 재주입 2행 추가 + 단계 6 격리 폴링 서술 정정) · 갱신 2026-08-06(격리 종결 전 벤더 상태 확인 강제 + 발행 실패 재시도 간격 도입 반영) · 갱신 2026-08-14(재시도 소진 시 벤더 확인 관문 배선 — 자동 승인·자동 실패 종결과 격리 사유 4종, 중복 승인 신호 단일화, 부분 취소 격리의 재고 보류) · 갱신 2026-09-11(관리자 격리 종결의 벤더 조회에서 부분 취소를 실패가 아닌 확인 불가로 분류) · 범위: 브라우저 checkout → confirm → outbox 발행 → pg-service 실제 PG사 호출 → 결과 수신·재고 정산 → 상태 폴링.
...
### 단계 1 — 주문 생성 (checkout, 동기)

> **목적**: 구매자·상품·금액을 확정해 주문(`PaymentEvent`)을 `READY` 로 만든다.
> **불변식**: 같은 `Idempotency-Key` 는 항상 같은 orderId 를 돌려준다.
...
### 단계 3 — 확정 명령 발행 (payment outbox relay, 비동기)
...
13. **중복 발행 방지 선점** — `PENDING → IN_FLIGHT` 원자 CAS, 실패 시 포기(다른 Worker 처리 중) (`claimToInFlight`)
...
15. **발행 완료** — `IN_FLIGHT → DONE` (`outbox.toDone`)
    - 1차 회복 경로: TX 롤백으로 PENDING 복귀한 건은 `OutboxWorker`(`@Scheduled` fixedDelay 5s)의 5초 주기 배치 재픽업으로 회수(`findPendingBatch`)
    - 보조 안전장치: `IN_FLIGHT` 5분 타임아웃 회수(`recoverTimedOutInFlightRecords`), Worker 크래시 등 드문 경로 대상

### 단계 4 — 실제 PG사 호출 (pg-service)
...
18. **inbox 상태 분기** (`PgConfirmService`)
    ...
    - terminal 재수신이면 저장 결과 재발행(`PgTerminalReemitService.reemit`, 벤더 재호출 금지)
19. **Worker가 처리** — `PgInboxImmediateWorker`가 채널에서 take
    - `PENDING`이면 `processPending`(**`PENDING→IN_PROGRESS` CAS, SKIP LOCKED**)
    - `IN_PROGRESS`면 `processInProgressZombie`
    - 이후 **실제 PG사 호출**(TX 밖, `PgVendorCallService.invokeVendor` → `PgConfirmStrategySelector` → Toss/NicePay/Fake) → `applyOutcome`(TX_B)
...
flowchart TD
    ...
    subgraph PAY1["payment — 주문 생성 (단계 1)"]
...
```mermaid
flowchart TD
    subgraph CONFIRM["confirm 진입 분기 (단계 2)"]
...
> 노란 노드(`종결 가드 재발행`)는 RDB DONE 커밋 후 Kafka 커밋 유실 시 재고 확정이 영구 유실되던 갭을 메운 경로다.
> 수신측 product-service 가 결정적 키로 멱등 흡수하므로 over-publish 는 무해, under-publish 만 위험한 비대칭을 이용한다.
...
| 격리(QUARANTINED) 결제 수동 종결 | 종결 직전 **벤더 상태를 반드시 한 번 조회**한다 — 승인으로 확인되면 종결을 거부하고 격리를 유지, 실패·확인 불가면 진행하되 조회 결과를 사유에 덧붙인다. **부분 취소는 실패가 아니라 확인 불가로 접는다** — 벤더가 대금 일부를 쥔 상태를 빨간 실패로 표시하면 운영자가 그대로 종결해 버린다. 화면 배지는 회색으로 떨어지고 원래 상태값이 옆에 그대로 찍히며, 종결 사유에도 `확인불가(PARTIAL_CANCELED)` 로 남는다. 종결 자체를 막지는 않는다 — 취소·환불 경로가 없어 막아도 시스템 안에서 할 수 있는 일이 없다. 이후 `decrement:done` 토큰이 있을 때만 재고 보상(유령 재고 방지) → `QUARANTINED`→`FAILED` CAS 전이 + 감사 기록. 조회·판정은 비가역인 재고 보상보다 앞에 둔다. 정상 결제를 살리는 DONE 복구는 후속 과제 | `QuarantineResolveUseCase` / `PgVendorStatusPort` / 관리자 화면 조회·종결 버튼 |
...
위 수동 복구 두 건은 "종결할지 되살릴지"를 사람이 판단해야 한다. 그 판단 근거를 화면에서 볼 수 있게 두 곳을 열었다.

| 보려는 것 | 화면 | 읽는 방식 | 한계 |
|---|---|---|---|
| 재시도 경과 — 몇 번, 언제 시도해서 소진됐는지 | 관리자 결제 상세의 시도 이력 카드 | pg-service 관리자 조회 API 를 렌더 시 호출. 이력은 새 테이블 없이 기존 재시도 발행 행 + 최초 명령 수신 시각으로 조립 | 발행 시각은 실제 벤더 호출 시각의 근사값. 좀비 회수가 예약된 재시도를 앞지르면 벤더 호출 없이 발행만 된 행이 남으므로, 종결 시각보다 늦은 행은 **예약됐으나 미실행**으로 갈라 표시한다. 두 워커 동시 진입 시 같은 회차가 여러 행에 나타날 수 있다 |
| 확정 재고 — 결제 실패로 재고가 제대로 풀렸는지 | 관리자 재고 화면 (상품 단위 목록) | product-service 목록 조회 API 를 페이지 단위로 호출 | **확정 수량만** 표시한다. 승인 진행 중이나 캐시가 발산한 구간에서는 실시간 판매 가능 여부(캐시 기준)와 다를 수 있어 화면이 그 사실을 밝힌다. 조작 기능은 열지 않았다 — 선차감이 도는 중 사람이 재고를 만지면 동시성 문제가 붙는다 |

**관측이 대응을 막지 않는다**: 결제 상세는 격리 종결·DLQ 재주입 버튼이 걸린 대응 화면이다. pg-service 가 죽거나 느려도 상세는 열리고 두 버튼은 그대로 동작하며, 이력 카드만 조회 불가로 표시된다.
...
- end-to-end 상세(단계 1~5 + pg-service 내부 deep-dive): [PAYMENT-FLOW.md](PAYMENT-FLOW.md)
