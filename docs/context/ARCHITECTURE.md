...
OutboxProcessingService.process(orderId)  [ImmediateWorker/OutboxWorker 공유 — 복구 사이클]
  ...
  Step 4: getPaymentStatusByOrderId(orderId) [PG 상태 선행 조회, no TX]
...
**PG 예외 분류:**
- `PaymentTossRetryableException` — PG 일시 오류, 재시도 가능 → `RETRY_LATER` 또는 `QUARANTINE`
- `PaymentTossNonRetryableException` — PG에 결제 기록 없음 → `ATTEMPT_CONFIRM` (confirm 시도)
- `PaymentGatewayStatusUnmappedException` — PG 응답 상태 매핑 불가 → `RETRY_LATER` 또는 `QUARANTINE`
