...
| `StockHoldRecoveryWorker` (`@Scheduled`) | `payment-service/.../infrastructure/scheduler` | 미회수 선차감 회수 (기본 60초 주기, batch 100). 종결된 결제의 잡음 상태 `stock_hold_record` 를 찾아 `StockHoldRecoveryUseCase` 로 되돌린다 — 결제 상태 판정은 쓰기 경로와 같은 리포지토리 빈으로 원본 DB 를 읽는다. `stock_hold_recovery.run_total` / `outstanding_count` 지표 (STOCK-GATE-PER-PRODUCT) |
...
| `redis-stock` (6380) | 재고 선차감 게이트 + Lua atomic dedup token (`StockCachePort`) | payment-service 단독. **STOCK-GATE-PER-PRODUCT 이후 상품 단위** — 키는 상품 기준 해시태그(`stock:{productId}` / `decrement:done:{productId}:orderId` / `compensation:done:{productId}:orderId`)라 한 상품의 키가 같은 슬롯에 모인다. confirm 진입은 주문 단위 선점(`acquireOrderLock`) 후 상품마다 `decrementAtomic(orderId, order)` 호출, 부족을 만나면 직접 차감분만 `rejectCompensate`(재고 복원 + 두 표시 삭제, 되돌리기 표시가 이미 있으면 복원 건너뜀)로 되돌리고 거절. 되돌리기 경로는 `compensateIfDecremented`(흔적 있을 때만) 사용. 결과 enum: `StockDecrementAtomicResult` (OK / ALREADY_DONE / INSUFFICIENT). product RDB 가 SoT, 본 캐시는 그것의 미러 — 부팅 직후 `scripts/seed-stock.sh` 가 mysql-product 에서 SELECT → redis SET 으로 시드. 부팅 외 발산 보정은 운영 단건 resync(`POST /admin/stock/resync/{productId}` → `StockCachePort.set`). 그 상품에 미종결 선차감 기록(`NOISE`)이 있으면 기본 거부하고 `?force=true` 로만 넘긴다 — 사전 조회는 force 와 무관하게 항상 선행하며 조회 실패도 거부다. 확인~덮어쓰기 사이 창은 막지 않고, 덮어쓴 뒤 같은 조회를 한 번 더 해 사후 건수가 사전보다 크면 겹침을 응답·경고로 드러낸다(`StockResyncOverlapStatus` OVERLAPPED/CLEAR/UNKNOWN, 재확인 실패는 UNKNOWN). AOF `appendfsync=always` 운영 (L2 race window 완화 trade-off) |
...
- **파티션 수가 컨슈머 병렬도의 상한이다.** 리스너 동시성을 올리든 인스턴스를 늘리든 그룹당 활성 컨슈머는 파티션 수를 못 넘고, 남는 스레드는 파티션을 배정받지 못해 논다. 운영 파티션은 3 이고 `pg-service` 는 리스너 동시성을 지정하지 않아 Spring 기본값 1 — 즉 **컨슈머 하나가 파티션 셋을 다 든다**. payment 측 손잡이는 `payment.kafka.events-confirmed.consumer.concurrency` (기본 1). 실측에서 이 상한이 처리량 천장이었다 (파티션·동시성 9 로 **1.86배**, 거기서 인스턴스를 2대씩 더 붙여도 1.09배 — `docs/archive/shared-resource-scaleout/COMPLETION-BRIEFING.md`). 승격 판단은 `TODOS.md` `[KAFKA-PARTITION-DEFAULT-PROMOTION]`
...
| 결과 대기 상태 (늦은 확정 결과 수용) | `PaymentEventStatus.AWAITING_RESULT` — 리컨실러 1차 임계 초과 IN_PROGRESS 의 도착지. 비종결이며 `canApplyConfirmResult` true 라 늦게 온 승인·실패·격리 결과를 그대로 적용한다. READY 에서 DONE 으로 가는 길은 열지 않는다(확정 진입 없는 결제가 떠도는 승인으로 완료되는 것을 막는다) |
| 리컨실러 두 단계 스캔 | `PaymentReconciler.scan` — 1차 `resolveInProgressToAwaitingResult`(기본 300초), 2차 `resolveAwaitingResultToQuarantine`(기본 900초). 두 전이 모두 `WHERE id, status` 조건부 UPDATE(0건이면 건너뜀, `payment_order` 미터치) + `PaymentCommandUseCase` 위임 경유로 감사 기록. 배치는 항목별로 실패를 격리하고 경합 스킵과 예외 실패를 나눠 센다 |
| 확정 결과 소비의 잠금 읽기 | `PaymentConfirmResultUseCase.handle` 첫 조회가 `findByOrderIdForUpdate` — 리컨실러의 자동 격리와 소비 경로의 덮어쓰기를 DB 레벨에서 직렬화한다 |
| 상태 예외 비재시도 분류 | `KafkaErrorHandlerConfig` — `PaymentStatusException` 을 비재시도 목록에 포함. 재시도해도 결과가 바뀌지 않는 거부가 백오프를 소진하며 컨슈머 스레드를 잡는 것을 막는다 |
...
|---|---|---|---|---|
| payment | (1) Lua atomic dedup token — 재고 차감/보상 멱등 (redis-stock, **상품 단위**: `decrement:done:{productId}:orderId` / `compensation:done:{productId}:orderId`). 되돌리다 실패한 차감은 `stock_hold_record` + 회수 주기 작업이 정리. (2) `JdbcPaymentEventDedupeStore` (`payment_event_dedupe` INSERT IGNORE) — **메시지 단위 dedupe** (PET-5 신설). 둘은 역할이 다름 | (1) Redis (redis-stock) / (2) MySQL (mysql-payment) | (1) Lua 안에서 재고 DECR/INCR + token 박기 atomic. (2) EOS 트랜잭션 안에서 RDB dedupe + RDB 상태 전이 + Kafka 발행이 원자 | **강함** — (1) Lua single-shot atomic, (2) EOS 트랜잭션 원자성 |
