...
| DLQ 임계("4회 초과 시 DLQ", L139) | pg_outbox 발행(Kafka publish) 실패 자체는 이 표가 말하는 4회 한도(`RetryPolicy.MAX_ATTEMPTS=4`)의 적용 대상인지 불명확 — MAX_ATTEMPTS 는 벤더 confirm self-loop 재시도 카운터(`attempt`)에 적용되는 것이지, outbox 발행(Kafka send) 자체의 폴링 재시도에는 회수 횟수 상한이 없어 보임(폴링이 `processedAt IS NULL` 이면 무한 재시도) | `RetryPolicy.java:42-43`(`MAX_ATTEMPTS=4`, "attempt >= MAX_ATTEMPTS 면 DLQ 경로" — 벤더 재시도 attempt 전용 Javadoc), `PgOutboxPollingWorker` 회수 로직에 attempt 카운트 참조 없음(테이블 문맥상 발행 실패는 무한 폴링 재시도로 보임) | Task 15/16 반영 전 소스 재확인 필요 — "4회 초과 시 DLQ" 문구가 발행 실패 케이스에도 정확히 적용되는지, 아니면 attempt 카운터 전용 문구가 표에 잘못 끼어든 것인지 판정 | **S2 후보** (내부 모호성 — 확정 판정은 Task 14 재확인) |
