...
**완료 결과**
> topic 결정표(문체 수정 기준·재발 방지 결정) 4항목 + doc-review 보강 1항목, 5개 파일군에 1:1 반영 완료.
> - **`_shared/checklists/ship-ready.md`**(topic "재발 방지" 결정) — documentation sync 절에 3항목 추가: 3분류 삭제 룰 적용(대장 문서 ✅ 잔존 0, 보존 결정분 제외) / 헤더 "최종 갱신"-본문 동기화 / 대장 문서가 `docs/archive/README.md`와 중복되는 완료 섹션 없이 슬림 유지.
> - **`context-update/SKILL.md`**(topic "재발 방지" + "사실 판정 근거" 결정) — "## 갱신 절차" 에 신규 "### 5. 완료 항목 정리(TODOS·CONCERNS 등 대장 문서)" 절 추가: (a) 전체 삭제 (b) 혼합 항목은 해소분 문장만 제거 (c) 수용된 한계·회피된 우려 표 보존 3분류 명문화 + 헤더-본문 동기화 원칙 + "사실 서술의 근거는 소스 파일:라인만 인정, 다른 문서·archive briefing 인용으로 사실 확정 금지"(게이트 1R 이 실증한 기준 예문 사건 — 표본 #12, CONFIRM-FLOW §3 stale 인용이 위키의 참인 문장을 뒤집을 뻔한 사례 — 를 원칙으로 승격).
> - **`workflow-ship/SKILL.md`**(topic "재발 방지" 결정, 게이트 1R major 복원분) — B2 Context 문서 갱신 절에 1문장 추가: 대장 문서에 완료 항목이 남았으면 `context-update` 3분류 룰 적용 + 헤더 동기화를 ship 마무리 절차에서 직접 트리거하도록 명시(`ship-ready.md` documentation sync 항목과 상호 참조).
> - **`_shared/conventions/writing.md`**(topic "문체 수정 기준" 결정) — 기존 "문체" 절(`~다.`/명사형 종결, 문장 길이와 리스트)에 새 섹션 2개를 통합: "평가·번역투 표현 금지"(평가 형용사→사실·수치, 번역투→구체 동사, topic 기준 예문 그대로 인용) + "문장 밀도 — 단정문 연발 대신 연결어미, 긴 불릿은 하위 뎁스"(관련 절은 연결어미로 자연스러운 길이, 다시 길어지면 기존 리스트 전환 원칙 적용, 긴 불릿은 재편 없이 하위 뎁스 분해). 하단 "기존 문서 수정 시" 체크리스트에도 대응 항목 2개 추가 — 기존 규칙(명사형 종결·표·헤더 중심)과 충돌 없이 같은 절 안에 병존.
> - **`doc-review/SKILL.md`**(topic 결정표 밖이나 검증 전략 3항 "doc-review 에 문체 기준 적용"의 실행 전제라 포함) — 관점 1(규격 준수) 체크 항목 표에 "평가·과시 형용사·번역투 없음" + "문장 밀도 적절"(긴 불릿 하위 뎁스 포함) 2행 추가, `writing.md` 신규 절과 1:1 대응.
> - **`docs/DOCS-CONSISTENCY-OVERHAUL-DIAGNOSIS.md`**: §4.1.4 L-14(CONCERNS.md 3분류 (b) 혼합 항목 모범 사례) 행을 "인용 가치" → "인용 완료 — `context-update` SKILL.md '완료 항목 정리' 절의 예시 근거로 반영"으로 갱신 + 리포트 헤더 "최종 갱신"에 Task 18 요약 추가.
> - 각 파일 기존 구조·표 형식 그대로 유지 — 새 섹션 남발 없이 기존 절(documentation sync / 갱신 절차 / B2 / 문체 / 관점 1 표)에 항목만 추가하는 최소 diff. 5개 파일 모두 `docs/context/**` 밖(스킬·체크리스트·컨벤션 문서)이라 이번 토픽의 진단·정정 대상(§4.1~4.5)과 겹치지 않음 — DIAGNOSIS.md 의 §4 페이지별 판정을 갱신할 대상은 없고, L-14 참조 1건만 종결.
> `./gradlew test` 대상 아님(스킬/체크리스트 문서만 변경, 코드 무변경).
