...
ship 1차에서 reviewer 는 revise(major 2), domain-expert 는 **pass**(findings 없음)를 냈다. domain-expert 는 배선의 도메인 위험을 코드로 대조하고 경합 통합 테스트 10회와 양쪽 서비스 테스트를 직접 실행해 확인했다 — 자동 확정으로 돈이 새는 경로가 새로 열리지 않았고, 전이 반영 가드가 세 갈래 모두에 걸렸으며, 양쪽 서비스의 부분 취소 사유 문자열이 일치하고, 격리 도달 카운터의 멱등 지점이 유지됐다는 것이 요지다.

| finding | severity | 처리 |
|:---:|:---:|:---:|
| 관문 클래스 Javadoc 이 "호출 주체는 후속 Phase 에서 연결 예정"이라는 미배선 시절 문구를 그대로 두고 있다 — 이번 작업이 그 연결을 했다 | major | 채택 — 배선 후 상태로 정정 |
| `ConfirmedEventPayload.quarantinedWithAmount` 가 고아가 됐다 — 유일한 호출처였던 옛 대기열 격리 페이로드 빌더가 삭제됐다 | major | 채택 — 삭제. 결제 쪽이 격리 결과에서 금액을 쓰지 않는 것(금액 검증은 승인 경로 한정)을 확인해 동작 변화가 없음을 먼저 대조했다 |
