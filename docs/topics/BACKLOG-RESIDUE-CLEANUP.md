...
| 항목 | 결정 | 이유 |
|---|---|---|
| 미사용 선점 메서드 | `PaymentOutboxUseCase` 의 `claimToInFlight`·`incrementRetryOrFail` 과 대응 테스트를 제거한다 | 프로덕션 호출처 0. 남겨두면 재시도 정책이 동작하는 것처럼 오독된다. 아래 유래 확인 결과 미완성 회귀가 아니라 의도된 단순화의 잔재다 |
...
대장은 이 경로를 "의도된 단순화인지 미완성 회귀인지 확인 필요"로 등재해 뒀다. git 이력으로 확인한 결과 **의도된 단순화의 잔재**다.

예전에는 payment 가 PG 를 직접 호출했고, 그 흐름을 `OutboxProcessingService.process()` 가 담당했다. 거기서 두 메서드는 각자 역할이 있었다.

- **별도 트랜잭션 선점** — 외부 PG 호출은 오래 걸리므로 선점 표시를 먼저 확정 커밋해야 그 사이 다른 워커가 같은 주문을 집어가지 않고, 호출 도중 프로세스가 죽어도 진행 중 표시가 남아 타임아웃 회수 대상이 된다. 긴 외부 호출을 감싸는 구조라 선점과 호출의 트랜잭션 분리가 필요했다.
- **재시도 횟수 증가** — 횟수가 곧 분기 조건이었다. 0 이면 바로 승인 요청, 1 이상이면 이전 요청이 PG 에 도달했을 수 있어 상태를 먼저 조회하고 판단했다.

MSA 전환(`0465ed0e`, 2026-04-21)에서 PG 직접 호출을 pg-service 로 넘기며 `OutboxProcessingService` 를 삭제했다 — 커밋 메시지에 "PG 직접 호출·RetryPolicy·FCG 로직은 pg-service 이관 대상"으로 명시돼 있다. 그 자리를 대신한 `OutboxRelayService.relay()` 는 Kafka 발행 한 번으로 끝나 짧으므로, 선점을 저장소 포트로 같은 트랜잭션 안에서 직접 호출하는 형태로 단순해졌다. 유스케이스의 두 메서드는 그때 호출자를 잃었다.

따라서 되살릴 물건이 아니다. 나중에 발행 실패에 간격을 넣기로 하면 지금 구조에 맞게 새로 설계해야 한다 — 이 메서드들은 PG 직접 호출을 전제로 만들어졌다.
