...
flowchart TD
    ...
    STATUS -->|FAILED| COMP["PaymentEvent IN_PROGRESS -> FAILED stock.restore 이벤트 발행 (Phase-3.3)"]
...
flowchart TD
    ...
    DLQTERM -->|No| QUAR["pg_inbox QUARANTINED reasonCode RETRY_EXHAUSTED pg_outbox events.confirmed QUARANTINED + stock.events.compensation row INSERT"]
...
- **정상 deserialize**: 4회 재시도 소진 메시지 → pg_inbox FOR UPDATE 후 terminal이면 no-op, 아니면 `QUARANTINED` 전이 + `pg_outbox`에 `payment.events.confirmed(status=QUARANTINED, reasonCode=RETRY_EXHAUSTED)` · `stock.events.compensation` row INSERT(같은 TX) → AFTER_COMMIT 이벤트로 worker가 실제 Kafka 발행.
...
| 14 | 보상(`stock.restore`)은 UUID 키로 consumer 측 dedupe (상품 서비스 소유) | ADR-16, Phase-3.3 | Phase-3.3 EventDedupeStore 테스트 |
...
- **payment-service ↔ product / user 구간**: 조회성 호출은 동기 HTTP 허용(읽기 일관성 요구 차이). 상태 변경성 전파는 이벤트(`payment.events.stock-committed`, `stock.restore`).
...
- **DLQ 경로 (전용 consumer 분리)**: `PaymentConfirmDlqConsumer`가 `payment.commands.confirm.dlq`를 별도 listener로 구독하고 처리한다. 정상 `PaymentConfirmConsumer`와 분리해 관심사를 격리한다.
  ...
  - 같은 TX 안에서 `pg_outbox`에 `payment.events.confirmed(status=QUARANTINED, reasonCode=RETRY_EXHAUSTED)`와 `stock.events.compensation` 두 row INSERT.
