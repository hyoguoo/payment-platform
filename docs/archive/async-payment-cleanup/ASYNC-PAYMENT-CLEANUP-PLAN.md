...
- [x] Task 6: application.yml 기본 전략 변경
- [x] Task 7: 컨텍스트 문서 업데이트
...
**완료 결과**
> `application.yml`에서 `async-strategy: sync` → `outbox` 변경. `PaymentConfirmServiceImpl` `@ConditionalOnProperty`의 `matchIfMissing=true` → `false`. `application-test.yml`에 `spring.payment.async-strategy: sync` 추가하여 통합 테스트가 의도치 않게 outbox 전략으로 전환되지 않도록 고정. 단위 테스트 `matchIfMissing` 검증도 `isFalse()`로 수정.

---

### Task 7: 컨텍스트 문서 업데이트 [tdd=false]

**구현**
- `CONFIRM-FLOW-ANALYSIS.md`: Sync 플로우에서 `validateCompletionStatus` 제거 → `validateLocalPaymentRequest` 추가(TX 진입 전). Outbox 플로우에서 LVAL 단계 추가, `validateCompletionStatus` 제거, `WithOutbox` 메서드로 업데이트. 복구 스케줄러(Section 4) 삭제. 전략 기본값 업데이트.
- `CONFIRM-FLOW-FLOWCHART.md`: 동일 변경 사항 반영. 플로우차트 노드 수정 및 복구 스케줄러 다이어그램 삭제.

**완료 결과**
> `CONFIRM-FLOW-ANALYSIS.md`와 `CONFIRM-FLOW-FLOWCHART.md` 양쪽을 현행 코드 기준으로 업데이트. 복구 스케줄러 섹션 삭제, Outbox 플로우에 LVAL 및 `WithOutbox` 메서드 반영, Sync 플로우에 `validateLocalPaymentRequest` 반영, 기본 전략 outbox로 수정.
