...
### ship 코드 리뷰 (reviewer + domain-expert, 둘 다 pass)

- **findings 0건** (critical / major / minor 모두 0). 수정 dispatch·스킵 항목 없음.
- reviewer (yes 9 / no 0): 데드 제거 dangling 참조·미사용 import 0(`JPAQueryFactory` 빈 주입 필드 포함 정리), Task 3 `AbstractImmediateWorker`는 template method 패턴(워커별 차이는 추상 메서드 위임, 골격만 base로) — 기각한 "generic 워커 통합"과 다르며 line-by-line 동작 불변, Task 4 통합본이 기존 payment.mock 원본과 byte 동일, test gate `--rerun-tasks` 강제 재실행 통과.
- domain-expert (findings 0): 5커밋 동작 불변 소스 교차검증. Task 1 callVendor 5분기+vendorType 선택이 invokeVendor+applyOutcome 동일 본문으로 보존, Task 3 graceful shutdown 순서·getPhase·traceparent 이중 scope 동등 + Polling 2종 무변경(PITFALL 12 회피), Task 4가 order 중복 append 함정(R3, amount 대조 오염) 구조적 회피. EOS 통합 5 시나리오(정상 commit / abort+DLQ / 중복 INSERT IGNORE / multi-product idempotencyKey / QUARANTINED D7 가드) Testcontainers 강제 재실행 전부 PASSED.
