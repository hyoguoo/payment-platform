...
- **review 2라운드 반영** (2026-06-08): 루트 build.gradle 데드코드·오해 주석 정리(Critic F6). 게이트 단위화 이후 `def itTask = tasks.findByName('integrationTest')`와 빈 `if (itTask != null) { ... }` 블록(내부 주석 "리포트가 통합 exec도 합산"이 실제와 자기모순) 제거. afterEvaluate 진입 주석을 "jacocoTestReport·jacocoTestCoverageVerification 모두 단위 test.exec 기준으로 동일하게 평가, 통합 exec 합산 없음, 통합 정합성은 integration-test job pass/fail로 보호"로 정정. 기능 동작 불변(게이트 wiring 무변경). `./gradlew :payment-service:build :pg-service:build :product-service:build :user-service:build -x integrationTest` + `./gradlew :gateway:build :eureka-server:build -x test` 전부 BUILD SUCCESSFUL.
