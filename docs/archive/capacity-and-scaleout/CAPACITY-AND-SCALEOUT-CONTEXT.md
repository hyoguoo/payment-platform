...
| D5 | DLT suffix 갭 | 이 토픽 **첫 태스크**로 수정(test-first). **정합 방향 = 코드 상수 `EVENTS_CONFIRMED_DLQ`(`.dlq`)로 수렴** — `confirmedDlqKafkaTemplate` 은 이미 `.dlq` defaultTopic 을 박았으나 `DeadLetterPublishingRecoverer` 단일 인자 생성자의 기본 resolver(`topic + "-dlt"`)가 이를 무시하고 `payment.events.confirmed-dlt`(소문자 `-dlt`)로 발행 → `create-topics.sh` 미생성 + `auto.create.topics.enable=false` → 부재 블로킹. 처방 = recoverer 에 `.dlq` 고정 destination resolver 주입(실제 토픽명·시그니처는 plan 에서 확정) | 페이즈 2 e2e 중 예외 누적 시 consumer 블로킹 재발 위험 — 측정 안정화 선결 |
