...
그림의 "거절 되돌리기" 전이는 즉시 일어나는 것처럼 보이지만, 실제로 그 경로는 재고 캐시만 그 자리에서 되돌리고 기록은 건드리지 않는다 — 기록이 되돌림으로 닫히는 것은 재시도가 잡음을 새로 열거나, 재시도 없이 결제가 종결된 뒤 주기 작업이 집어갈 때뿐이라 이 전이는 늦어질 수 있다.
