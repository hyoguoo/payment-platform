...
| 운영과 검증 | 15 시드·토픽 스크립트 / 16a 이중 되돌리기 조합 / 16b 동시 중복 확정과 재시도 / 16c 수렴 체인과 정합 / 16d 방어 공백 메우기 / 17 라이브 검증 |
...
- [x] Task 17: 라이브 검증 (ship 단계에서 수행 완료)
...
- 신규 `StockGateConvergenceIntegrationTest`(16b 와 같은 `@DataJpaTest` + Flyway 활성 + `ddl-auto: validate`
  실 MySQL/Redis Testcontainers) — 다섯 시나리오 모두 새 프로덕션 코드 없이 GREEN. 강제 종료를 재현할
  별도 프로세스 킬 훅은 만들지 않았다 — 죽는 지점 이후의 호출을 안 하는 것만으로 재현되고, 훅이 필요한
  유일한 지점(되돌리기-닫기 사이)은 16b 가 신설한 `StockHoldReverter.beforeClose` 를 그대로 재사용했다
    - **상품 반복 도중 강제 종료** — 상품 하나만 `openHold`+`decrementAtomic` 한 뒤 나머지 상품은
      아예 건드리지 않고 멈춘다. `PaymentEvent.execute()`가 상품 반복 안에서 호출되지 않는 구조라
      두 상품 모두 `NOT_STARTED`로 남는 것을 실제로 단정한 뒤, 만료(`expirePayment`)를 태워 전부
      `EXPIRED`로 갈라짐 없이 전이되는지, 회수가 남은 기록(반복이 닿은 상품 하나) 만 정리하는지 확인
    - **되돌리는 도중 강제 종료** — 두 상품을 차감한 뒤 하나만 거절 전용으로 되돌리고 죽는다. 결제를
      재고 부족 실패로 종결시킨 뒤 회수를 태워, 이미 되돌린 상품은 흔적이 없어(`NO_DECREMENT`) 재고를
      건드리지 않고 기록만 닫고, 아직 못 되돌린 상품만 실제로 복원되는 것을 단정 — 이중 복원 없음
    - **회수가 되돌리다 강제 종료** — `StockHoldReverter`를 서브클래싱해 `beforeClose`에서 예외를
      던지는 크래시 버전으로 `StockHoldRecoveryUseCase`를 직접 생성(Spring 빈 아님, `new`)해 1회차
      회수가 캐시는 복원하되 기록을 닫기 전에 죽는 것을 재현. 기록이 잡음(NOISE) 그대로 남는 것을
      확인한 뒤 정상 회수 워커로 2회차를 태워, 흔적이 이미 없어 재고를 두 번 복원하지 않고 기록만
      닫히는 것(다음 주기에 다시 집힘)까지 단정
    - **선점을 쥔 채 강제 종료 후 수명 경과** — `StockCacheRedisAdapter`를 주문 락 수명 1초로 별도
      생성해 선점만 잡고 해제·상품 반복 없이 멈춘다. 수명이 남은 동안 재시도는 `ALREADY_PROCESSING`
      으로 물러나고, 1초 뒤 같은 주문번호 재시도(`PaymentTransactionCoordinator.decrementStock`,
      확정 TX 관련 협력자는 미사용이라 Mockito mock)가 선점을 다시 잡아 `SUCCESS`까지 완주하는 것을 확인
    - **정합** — 상품 하나에 확정(COMMITTED)된 주문 하나와 진행 중(NOISE)인 주문 둘을 만들어, (게이트
      값과 확정분만 반영한 가상의 상품 DB 값의 차이)와 (그 상품의 NOISE 기록 수량 합) 이 정확히 같은
      값으로 일치하는 것을 두 갈래로 교차 확인
- `./gradlew :payment-service:test` 680건 전체 pass(신규 5건 포함), checkstyle·spotbugs 클린

### Task 16d: 검증에서 드러난 방어 공백 메우기 [tdd=true] [domain_risk=true]

> 16a·16b 가 검증을 짜면서 찾은 두 가지다. 둘 다 지금 사고가 나지는 않지만, 방어가 코드가 아니라 상황에 기대고 있어 미래 변경에 취약하다.

**테스트 (RED)**

- **거절 전용 되돌리기에 되돌리기 표시가 이미 있으면 재고를 복원하지 않는다.** 표시 삭제는 그대로 수행해 재시도가 깨끗하게 시작되도록 한다
- 표시가 없으면 지금처럼 복원한다 — 정상 경로 회귀 확인
- **스키마 자동 생성 환경에서도 주문번호와 상품번호 유일 제약이 선다** — 같은 조합으로 두 번 기록하면 행이 하나만 남고 재오픈이 동작한다

**구현 (GREEN)**

- `lua/stock_reject_compensation.lua` — 되돌리기 표시 존재 시 복원을 건너뛰고 표시 정리만 수행
- `infrastructure/entity/StockHoldRecordEntity.java` — 유일 제약을 엔티티에도 명시해 마이그레이션과 자동 생성 스키마가 같아지게 한다

**근거**

- 16a 실측: 거절 전용은 무조건 복원이고 dedup 이 없다. 지금 안전한 이유는 표시가 아니라 주문 단위 선점과 상태 배타성이다. 순서를 바꿔 재현하니 이중 복원이 그대로 일어났다(100 → 100 → 110). 회수 작업의 대상 상태를 넓히거나 선점 범위를 바꾸면 방어선이 조용히 사라진다
- 16b 실측: 제약이 마이그레이션에만 있어 스키마를 자동 생성하는 통합 테스트 베이스에서는 서지 않는다. 그 베이스로 같은 조합을 두 번 기록하면 재오픈 대신 중복 삽입이 생긴다
...
**완료 결과** (2026-08-18, ship Phase B)

캐시·DB 볼륨을 전부 비우고 스택을 새로 띄워 다섯 항목을 모두 확인했다.

| 항목 | 결과 |
|:---:|:---:|
| 시드 키 이름 | `stock:{1}` `stock:{2}` `stock:{3}` 만 존재, 옛 형식 0건. 상품 DB 값과 일치하고 게이트가 읽어 결제를 통과시켰다 |
| 여러 상품 주문 | 상품 1(x2)·2(x3)·3(x1) 주문이 완주. 게이트가 100→98 / 100→97 / 5→4 로 상품별 차감, 표시 3개가 `decrement:done:{상품}:주문번호` 형식으로 생성, 기록 3행이 확정으로 전이. 상품 DB 잔고와 게이트가 셋 다 일치 |
| 회수 작업 | 9회 기동, 미회수 잔량 0 으로 수렴. 결과별 카운터 — 복원 3 / 흔적 없음 3 / 이미 처리됨 0 |
| 음수 가드와 격리 | 게이트를 인위로 부풀린 상태(잔고 1, 게이트 200)에서 5개 주문 → 결제는 승인됐으나 **잔고가 1 그대로 유지**(가드가 저장 거부). 격리 토픽 파티션 2 에 적재됐고 메시지 키가 `주문번호:상품번호` 조합. 이후 정상 주문이 그대로 완주해 파티션이 막히지 않음을 확인 |
| 지연 측정 | 상품 1개 중앙 25.6ms / 2개 32.3ms / 3개 36.3ms — **왕복 하나당 약 5ms**, 상품 수에 선형 |

**부수 확인** — 부분 차감 거절 경로가 실제로 돌았다. 상품 3에서 부족을 만나자 앞서 차감한 상품 1·2가 원래 값으로 정확히 복원되고 두 표시가 모두 삭제됐다. 이어서 같은 주문번호 재시도가 재고를 다시 차감했고(닫힌 기록이 잡음으로 재오픈), 그 차감을 회수 작업이 되돌려 게이트가 원상복구됐다 — 게이트 6~9라운드에서 다툰 재오픈 메커니즘이 실제로 작동하는 것을 관측했다.

**환경 문제 2건** (코드 무관) — 첫 기동에서 Testcontainers 잔여 컨테이너 45개가 메모리를 점유해 MySQL 두 대가 강제 종료됐고, 그 여파로 데이터 디렉터리가 손상돼 접속이 거부됐다. 라벨로 골라 정리하고 볼륨을 다시 초기화해 해소했다.

**발견 (범위 밖)** — `.claude/skills/payment-live-drill/references/setup.md` 의 시드 확인 명령이 아직 옛 키 형식(`stock:1`)을 썼다. 마무리 커밋에서 새 형식으로 맞췄다.

## 리뷰 처리

reviewer **revise**(major 2) / domain-expert **pass**(minor 3). critical 0.

| severity | finding | 처리 |
|:---:|:---:|:---:|
| major | 신규 Testcontainers 통합 테스트 4개에 `@Tag("integration")` 누락 — 단위 게이트에 편입되고 통합 태스크에서는 빠져 CI 통합 job 의 보호망 밖에 있다 | 채택 — 태그 부여 후 양쪽 태스크에서 위치 확인 |
| major | `StockHoldRecordEntity.openNoise()` 호출처 0. 삽입이 네이티브 쿼리로 처리되면서 죽은 코드가 됐고, Javadoc 의 "외부 호출자는 이 factory 를 거친다" 서술이 실제와 어긋나 오도한다 | 채택 — 메서드와 Javadoc 제거 |
| minor | 상품 반복 중 예외를 캐시 장애와 기록 저장소 장애 구분 없이 같은 이벤트 타입으로 로그한다 — 사고 조사 시 원인 축이 잘못 가리킨다 | 채택 — 실패 출처를 구분해 남긴다. 판정(격리 전이)은 그대로 |
| minor | 거절 경로가 캐시만 되돌리고 기록은 회수 작업이 나중에 닫는다. 설계 다이어그램은 즉시 전이로 그려져 문자 그대로는 어긋난다 | 부분 채택 — 안전성은 테스트로 고정돼 있어 코드는 그대로 두고, 설계 문서에 지연 가능성을 한 줄 보강 |
| minor | 라이브 검증(Task 17) 미수행 — ship 단계로 명시 위임된 상태 | 조치 불필요 — Phase B 에서 수행한다 |

도메인 검토는 다섯 되돌리기 주체와 사이클 토큰, 표시 선점의 상호작용을 코드까지 추적해 이중 복원·초과 판매 경로를 찾아봤으나 열리지 않았다고 판정했다. reviewer 도 설계 결정 27개를 코드와 대조해 어긋남을 찾지 못했고, 옛 주문 단위 포트 메서드와 스크립트가 참조 0 으로 완전히 걷힌 것을 확인했다.
