...
**완료 결과 — 2026-04-24**
- payment-service `application.yml`: `spring.kafka.template/listener.observation-enabled=true` 추가 + `management.tracing.sampling.probability=${TRACING_SAMPLING_PROBABILITY:0.0}` + `management.otlp.tracing.endpoint=${OTLP_TRACING_ENDPOINT:http://localhost:4318/v1/traces}` 추가.
- pg-service `application.yml`: 동일 패치.
- product-service `application.yml`: 동일 패치 (`spring.kafka.template/listener.observation-enabled=true` + `management.tracing.*`).
- payment-service `KafkaProducerConfig`: commandsConfirmKafkaTemplate·stockCommittedKafkaTemplate·stockRestoreKafkaTemplate 수동 빈 3개에 `template.setObservationEnabled(true)` 추가 (YAML 만으로는 수동 빈에 미반영).
- pg-service `KafkaProducerConfig`: confirmedEventKafkaTemplate·commandsConfirmKafkaTemplate·commandsConfirmDlqKafkaTemplate 수동 빈 3개에 동일 추가.
- product-service: `KafkaTemplate` 오토컨피그 기본 빈 사용 → YAML 설정만으로 충분, 코드 패치 불필요.
- `scripts/smoke/trace-header-check.sh` 신설 (executable +x) — `payment.commands.confirm` 토픽 record header `traceparent:` grep, exit 0/1.
- `docs/context/CONVENTIONS.md` 관측성 규약 섹션 추가 — `observation-enabled` 필수 + Micrometer Tracing 기본값 규약 2줄.
- `./gradlew test` 전체 PASS(회귀 없음).

---

### T3.5-07 — QUARANTINED 재고 복구 경로 철거 (B-1 + C-4 PaymentReconciler TODO)

- **제목**: QUARANTINED는 홀딩 상태 — Redis INCR/재고 복구 시도 경로 전면 삭제. 재고 복구는 FAIL 경로만.
- **목적**: 도메인 규칙 확정 — QUARANTINED(벤더 상태 불명)는 **판정 보류** 상태로 재고를 DECR 상태 그대로 홀딩. 실제 실패 판정은 운영자 수동 전이(QUARANTINED → FAIL)로만 수행하며, 이때 기존 `FailureCompensationService` 경로(`stock.events.restore` 발행)가 재고를 복구한다. 따라서 FCG·DLQ·Reconciler 경로에서 QUARANTINED 대상 Redis INCR 호출은 전부 버그.
- **tdd**: true
- **domain_risk**: true
- **depends**: [T3.5-13]
- **scope**: Java 코드 삭제만. DB 스키마는 건드리지 않음 (ddl-auto는 개발 단계이므로 엔티티 필드 제거해도 기존 `quarantine_compensation_pending` 컬럼은 DB에 NULL로 잔존 허용 — Hibernate가 mapped 컬럼만 검증하므로 reverse direction 안전).
- **산출물 (삭제)**:
  - `PaymentReconciler.rollbackQuarantinedDecr()` 메서드 + `scan()` 내 호출 + TODO 주석
  - `QuarantineCompensationHandler`: `attemptStockRollback()`, `rollbackStock()`, `retryStockRollback()`, `clearPendingFlagInTx()` + `StockCachePort` 의존 + `QuarantineEntry` enum (두 진입점 동작이 동일해지므로 파라미터 제거)
  - `QuarantineCompensationScheduler` 전체 파일
  - `PaymentEvent` 도메인: `quarantineCompensationPending` 필드 + `markQuarantineCompensationPending()` / `clearQuarantineCompensationPending()` 메서드 + `quarantine()` 내부의 플래그 set 로직
  - `PaymentEventEntity`: 동 필드 (컬럼 매핑 제거, DB 컬럼은 유지)
  - 위 경로에 묶인 테스트 (`QuarantineCompensationSchedulerTest` 등)
- **산출물 (수정)**:
  - `QuarantineCompensationHandler.handle(orderId, reason)` — QUARANTINED 전이만 수행, Redis 연산 없음
  - `PaymentConfirmResultUseCase.handleQuarantined()` — 진입점 분기 파라미터 제거
- **산출물 (신규 테스트)**:
  - `PaymentReconcilerTest` — QUARANTINED 레코드가 있어도 `StockCachePort.rollback` 0회 호출 (불변식)
  - `QuarantineCompensationHandlerTest` — QUARANTINED 전이 후 `StockCachePort` 접촉 0회 (불변식)
- **Rule 2 발동 여부**: 없음 (순수 Java 코드 삭제, Flyway 추가 없음).
- **완료 결과 (2026-04-24)**: QuarantineCompensationScheduler 전체 삭제. QuarantineCompensationHandler StockCachePort 의존·QuarantineEntry·attemptStockRollback/rollbackStock/retryStockRollback/clearPendingFlagInTx 제거 → handle(orderId, reason) 단순 TX 전이만. PaymentEvent quarantineCompensationPending 필드·markQuarantineCompensationPending/clearQuarantineCompensationPending/quarantine 내 플래그 set 삭제. PaymentEventEntity 컬럼 매핑 제거(DB 컬럼 유지). PaymentEventRepository findByQuarantineCompensationPendingTrue 인터페이스·구현·Fake 전부 제거. PaymentConfirmResultUseCase QuarantineEntry 파라미터 제거. PaymentTransactionCoordinator markQuarantineCompensationPending 호출 2곳 삭제. 불변식 테스트 2케이스 추가(PaymentReconcilerTest: rollback 0회 / QuarantineCompensationHandlerTest: StockCachePort 접촉 0회). 271/271 PASS, 회귀 없음.
