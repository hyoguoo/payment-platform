...
### 6.5 doc-review 재검수 라운드 2 — 4관점 잔존 반영 (위키 파일 수정 + 메인 저장소 커밋)

수정 2차(§6.4) 이후 재검수에서 4관점(규격/정확성/서사/독자) 전반에 걸쳐 파일:라인 특정된 잔존 지적을 반영한다. R2 라는 명칭은 doc-review 재검수 2회차를 가리키며, 반영은 위키(파일 수정까지만, 커밋은 사용자) + 메인 저장소(README/GUIDE — 이 커밋에 포함) 양쪽에 걸친다.

**위키**:
- `structured-logging.md` — L125 존재하지 않는 `STOCK_INCREASE_REQUEST` → 실존 EventType `STOCK_DECREASE_FAIL`(`payment-service EventType.java:84` 확인)로 교체. L8·L254 백틱 리포 경로(`docs/context/TODOS.md`) → GitHub URL(`[TODOS.md](https://github.com/hyoguoo/payment-platform/blob/main/docs/context/TODOS.md)`) 2곳.
- `trace-propagation.md` — `traceid` 소문자 표기 13곳 → `traceId` 통일(실제 MDC 키는 logback-spring.xml `%X{traceId}` 기준, 앵커 슬러그 `#traceid-가-머무는-두-threadlocal` 1곳은 GitHub Wiki 자동 소문자 슬러그라 그대로 유지). L327 백틱 경로(`docs/smoke/trace-continuity-check.md`) → GitHub URL. L7 `<img alt="image" src="..."/>` → `![image](...)` 마크다운 표준 이미지 문법. L250-251·253-254·281-282 하드랩(문장 중간에서 줄바꿈된 것) 3곳을 문장 경계로 재배치. L274-277 "best-effort"/"레이어 격리" 2개 불릿 내부 2문장 걸침을 각각 하위 뎁스 sub-bullet 2개로 분해.
- `compensation-tx.md` — L239-240 내부 토픽명 2곳("stock-compensation-recovery 토픽"/"stock-compensation-other-paths 토픽") → 날짜+작업명 자연어("2026-05-08, 보상 멱등 토큰 원자화 작업" 류)로 치환. "최종 일관성 보장 원리" 절의 폐기 고지 가드 문구가 원래 "2~5번만" 폐기라고 명시해 1번 항목(Outbox PENDING 기록)을 커버하지 못하던 것을 "1번의 기록 자체는 현재도 유효하나 재조회 역할 서술은 복구 사이클 전제라 함께 폐기" 로 보정. 절 표식 "(Phase 5 모델)"(4곳)과 "(Phase 5 모델 기준)"(2곳) 두 형태 혼용을 "(Phase 5 모델)" 로 통일.
- `pg-confirm-flow.md` — "이 세 단계 분리는 2026-05-09 도입" 역사 문장을 세 단계(listener/Worker/릴레이 Worker) 정의 문장 뒤로 재배치. FCG 한글명 "최종 확정 게이트" → "격리 전 최종 확인"(compensation-tx.md/retry-recovery.md 와 통일, 2곳: 헤더 + 본문). L143 QUARANTINED 불릿을 다른 두 불릿과 같은 명사형(`**QUARANTINED — 운영자 검토 격리 상태**`)으로 통일. L273-274 하드랩 1곳 재배치. L208(신규(absent))·L212(보정 경로 PENDING 우회) 두 불릿(각 280자대)을 하위 뎁스 sub-bullet 로 분해. L82 SoT 최초 등장에 "(Source of Truth, 단일 진실원)" 풀이 추가. CAS 풀이(`Compare-And-Set 방식`)를 L217(TX_A 상세 서술)에서 실제 최초 산문 등장 위치인 L119(TX_A 짧은 잠금 불릿)로 이동, L217 은 축약 참조만 남김.
- `message-delivery-and-dedupe.md` — L197 "RDB layer — ... 3단으로 분리된다" 서술형 불릿을 다른 행(Redis layer)과 같은 명사형으로 통일. L145-147 종결 가드 설명 하드랩 2문장을 문장 경계로 재배치.
- `stock-cache-recovery.md` — L220-221 에러 핸들러 범위 설명 하드랩을 2문장 경계로 재배치.
- `msa-transition.md` — L302 "재고 복원 가드" → "재고 복구 가드"(문서 전체 용어 통일, `outbox-pattern.md`/`state-management.md` 등과 일치).
- `outbox-pattern.md` — L216 "서버 장애 상황" → "서버 크래시 상황"(다른 위키 페이지의 "서버 크래시" 표기와 통일). L124 SoT 최초 등장 풀이, L202 OTel 최초 등장 풀이(`OpenTelemetry, 관측성 표준`) 각 1곳 추가.
- `retry-recovery.md` — L6 FCG(`격리 전 최종 확인, Final Confirmation Gate`) 풀이 + L9 DLQ(`Dead Letter Queue, 격리 큐`) 풀이를 배너 최초 등장 위치로 이동(기존 L121 풀이는 약어만 재사용). `architecture.md` 의 FCG 상세 포인터를 `state-management` 에서 `pg-confirm-flow`(가장 상세한 FCG 섹션 보유)로 통일.
- `architecture.md` — FCG 포인터 통일(위와 동일 건) + 디렉토리 트리 주석의 `DLT` 에 `(Dead Letter Topic, 격리 토픽)` 풀이 추가.
- `async-outbox.md` — L315 VT(스레드 선택 비교 표, Tomcat 행) 최초 그 표 안에서의 등장에 `(Virtual Thread, 가상 스레드)` 풀이 추가.

**메인 저장소**(이 커밋에 포함):
- `README.md` — 표 셀 2곳(L20 "가상 스레드 워커"→"가상 스레드 Worker", L25 "워커 VT"/"릴레이 워커"→"Worker VT"/"릴레이 Worker") + mermaid 노드/엣지 라벨 4곳("가상 스레드 워커"/"워커가 자기 스레드에"/"실시간 워커"/"다른 워커가 처리 중") → "Worker" 로 통일.
- `docs/context/PAYMENT-FLOW-GUIDE.md` — mermaid 노드 라벨 2곳(§B-1·§B-2 "[19] 워커: ...") → "[19] Worker: ...". 중간점(·) 4곳(§B-1 "위변조·상태 가드" 2곳, "결과 확정·정산" 1곳, §B-2 "rollback·offset" 1곳) → "/". L377 `attempt` 한도/DLQ 불릿(6~7문장 단일 불릿) 을 6개 하위 뎁스 sub-bullet 로 분해. L143 "승인(APPROVED)" 불릿(2개 조건 분기 포함)을 2개 sub-bullet 로 분해.

**부수 발견(Rule 1, 이 커밋에 함께 반영)**: 위 sweep 도중 `docs/context/INTEGRATIONS.md` "관측성 통합" 표의 Loki 행이 "Logback `LogstashEncoder` + LogFmt → Promtail/직접 push" 로, 이미 코드에서 완전히 제거된 `LogstashEncoder`(전 서비스 `logback-spring.xml` grep 0, structured-logging.md 위키가 이미 "Console appender + Promtail/Loki 대체" 로 기록한 것과 동일 축)를 현재형으로 서술하고 있음을 신규 발견 — Task 19 자체의 stale 마커 grep(Elasticsearch/Logstash) 대상이라 이 커밋에서 "Loki | Logback Console appender(LogFmt) stdout → docker 로깅 드라이버(`com.hyoguoo.loki.enable` 라벨) → Promtail → Loki push" 로 직접 정정, 헤더 "최종 갱신" 동기화.

**스킵 결정 추가 없음** — §6.3 의 3건(표 포맷/msa-transition 알람 그룹/README·GUIDE 괄호 병기)은 §6.4 에서 이미 확정 처리됨, 이번 라운드는 신규 스킵 없이 지적 전건 반영.

**./gradlew test** — 문서 전용 변경(위키 + README + GUIDE + INTEGRATIONS.md)이나 최종 스냅샷으로 1회 실행해 회귀 없음 확인(§7 참고).

## 7. 최종 검증 스윕 결과 (Task 19 종결)

### 7.1 기계 검사

- **상대 링크 검사** — 대상: `docs/context/**` + `docs/smoke/**` + `README.md` + `docs/STATE.md` + `docs/topics/DOCS-CONSISTENCY-OVERHAUL.md` + 본 리포트/PLAN 자기 자신. 실제 파일 링크(`.md` 확장자 또는 경로 구분자 포함) 전건 대조 — **0건 파손**(본 리포트 §4.4.10 인용문 안의 상대 경로 오타 1건 발견 → `context/CONFIRM-FLOW.md` 로 즉시 정정). 위키 25페이지 내부 링크(슬러그 기준) 전건 대조 — **0건 파손**. 참고: `docs/archive/**` 하위 과거 토픽 문서들의 `topics/*.md` 상호 링크 파손은 다수 존재하나, 이는 ship 시점에 아카이브로 이동되며 생기는 기존 관행(토픽 완료 후 `docs/topics/` 원본이 정리됨)이고 이 토픽이 만든 회귀가 아니며 대상 범위(에이전트 워킹 문서) 밖이라 스코프 아웃 — 별도 후속 없음.
- **stale 마커 grep** — 대상 문자열: `RETRYING`(현재형) / `StockOutbox`(현재형) / payment 측 `EventDedupeStore`(현재형) / `RecoveryDecision`(현재형) / `REQUIRES_NEW` 선점(현재형) / `Elasticsearch`·`Logstash`(현재형) / `MaskingPatternLayout`(현재형) / `워커`(README/GUIDE 대상) / `를 통해`(README/GUIDE/위키 대상). `docs/context/**` + `docs/smoke/**` + `README.md` + 위키 25페이지 전체 grep 결과, 잔존 매치는 전부 "과거"/"폐기"/"삭제됐다"/역사 절 안 서술이거나 코드와 일치하는 정확한 현재 서술("REQUIRES_NEW 아님"/"pg-side EventDedupeStore" 등) — **현재형 stale 서술 0건**. 예외 1건 신규 발견·정정: `docs/context/INTEGRATIONS.md` 의 `LogstashEncoder` 현재형 서술(§6.5 부수 발견, 이 커밋에서 정정 완료). `워커`/`를 통해` 는 `docs/context/**` 전반(CONFIRM-FLOW/PAYMENT-FLOW/ARCHITECTURE/STRUCTURE/PITFALLS/smoke 등)에 기존 확립된 표기로 다수 잔존하나, 이번 라운드의 대상은 명시적으로 README/GUIDE(+ 위키 sweep, §6.4)였고 `docs/context/**` 전역에 대한 "워커→Worker"/번역투 전면 치환은 이 topic 의 plan 결정(§6.1 문체 관점은 위키+README+GUIDE 대상)범위 밖이라 스코프 아웃. `Benchmark-Report.md`(위키) 의 `워커`/`를 통해` 각 1건은 시점 기록 문서라는 이유로 §6.4 에서 이미 명시적으로 범위 밖 처리됨 — 재확인 결과 그대로 잔존, 의도된 예외.

### 7.2 doc-review 4관점 재검수 결과

§6.1(라운드 1) 4관점 전건 FAIL → §6.2/§6.4(수정 1·2차) 반영 → §6.5(라운드 2 재검수) 로 규격 잔존 지적 전건(위키 11파일 세부 + README/GUIDE mermaid·중간점·장문불릿) 반영 완료. 최종 판정:

| 관점 | 최종 판정 |
|---|---|
| 규격 준수(mermaid 표기·중간점·하드랩·불릿 뎁스·약어 풀이 등) | **PASS** — §6.5 잔존 15건 전건 반영, 잔여는 §7.3 스킵 사유 기록분만 |
| 기술 정확성(소스 대조) | **PASS** — §6.2 8건 + §6.5 부수발견 1건(INTEGRATIONS.md) 전건 해소, 이번 라운드 신규 정확성 지적 없음 |
| 동일 파일 서사 정합 | **PASS** — §6.2 2건 해소 유지, §6.5 에서 compensation-tx 가드 문구·Phase 5 표식 통일로 추가 보강 |
| 독자 관점(중복·밀도·용어 일관성) | **PASS** — §6.2 5건 + §6.4 다수 + §6.5 나머지(하드랩 5곳·장문 불릿 4곳·용어 통일 6곳) 전건 반영 |

### 7.3 스킵 사유 (최종 확정, §6.3 이후 신규 없음)

1. 표 구분선·패딩 정규화 — `writing.md` "표 포맷 작성자 입력 그대로 유지" 원칙과 충돌, 유지.
2. `msa-transition.md` 알람 규칙 "3그룹" 지적 — 소스 재확인 결과 4그룹이 정합, 지적 근거가 문서 상호 인용이라 §0.3 원칙상 기각.
3. README 이미지 5건(`<img width>`) — 크기 지정이 순수 마크다운으로 불가해 유지. (참고: 위키 `trace-propagation.md:7` 는 크기 지정이 필요 없어 이번 라운드에 `![image](...)` 표준 문법으로 전환 — README 와는 별개 판단.)
4. GUIDE 괄호 병기(`(PaymentController.checkout)` 류) — 코드 식별자 부가 표기로 가이드 문서 목적에 부합, 유지.
5. `:---` 좌측 정렬 표 셀 — 표 포맷 보존 원칙, 유지.
6. "한 줄에 2문장" soft 지적 약 20건 — `writing.md` "한 줄에는 **가능한** 하나의 문장" 문구가 절대 금지가 아닌 권고로 해석되는 지점이라, 극단 사례(GUIDE L377, 6~7문장 단일 불릿)만 하위 뎁스 분해로 수정하고 나머지 경미한 2문장 병존은 수용.
7. `pg-confirm-flow.md` 시퀀스 다이어그램 메시지 안 중괄호(`{orderId, paymentKey, ...}`) — 코드 페이로드 구조 표기이지 mermaid 라벨 금지 문자(따옴표 등)와 다른 범주라는 판정 유지, 변경 없음.
8. `docs/archive/**` 링크 파손(§7.1) — 아카이브 고유 관행, 이 토픽 범위 밖.
9. `docs/context/**` 전역의 기존 "워커"/"~를 통해" 표기 + `Benchmark-Report.md` 의 동일 표기 — 이번 라운드 대상(README/GUIDE/위키 11파일) 밖, §6.4 확정 유지.

### 7.4 진단 리포트 전 항목 종결 확인

§4.1~4.5(Task 2~6 진단)은 각각 Task 7~17 정정 단계에서 종결 마킹 완료(§4.1.1/§4.1.2 는 Task 7, §4.1.3/§4.1.4 는 Task 8, §4.2 는 Task 9~10, §4.3 는 Task 11~12, §4.4/§4.5 는 Task 13~17). §5 완료 기준 대조도 전항목 충족. §6(doc-review)은 본 절(§6.5)로 라운드 2 까지 종결. 미종결 항목 — **0건** (§7.3 의 스킵 9건은 "의도적 미반영" 판정이며 "미종결"이 아니다 — 전부 사유·근거 기록 완료).

### 7.5 최종 검증 요약

- 기계 검사: 상대 링크 0건 파손(스코프 내) / stale 마커 0건(스코프 내, INTEGRATIONS.md 1건 발견 즉시 정정)
- doc-review 4관점: 전 관점 **PASS** (§7.2)
- 진단 리포트 항목: 전건 종결, 미종결 0건 (§7.4)
- `./gradlew test` — 문서 전용 변경(코드 무변경)이나 회귀 없음 최종 확인 위해 1회 실행 (실행 결과는 PLAN.md Task 19 완료 결과에 기록)

### 7.6 ship 코드 리뷰 게이트 finding 반영 (2026-07-07)

Task 19 종결 이후 ship 단계 코드 리뷰 게이트(reviewer pass — minor 2건, domain-expert pass — minor 4건) 결과 findings 6건 전건 채택·반영. 스킵 0건. 근거·처리 상세는 `docs/DOCS-CONSISTENCY-OVERHAUL-PLAN.md` "## 리뷰 처리" 절 표 참고. 요약:

- **위키**(파일 수정까지만, 커밋은 사용자): `cross-validation.md`(L36·39·110 mermaid 화살표·중간점 정리)·`event-driven-choreography.md`(L256 동일)·`structured-logging.md`(L23·25·26 "~을 통한" 관형형 → 동사형)·`pg-confirm-flow.md`(L17·L247 "실측" 창작 서술 정정 — `docs/archive/pg-confirm-listener-split/` 에 측정 기록 부재 확인)·`architecture.md`(presentation 트리 주석 "Kafka consumer 가 호출" → "HTTP Controller 가 호출", `PaymentController`/`ConfirmedEventConsumer` 소스 재확인).
- **메인 저장소**(이 커밋에 포함): `docs/context/TODOS.md` `[PAYMENT-STATUS-TRIGGER-DETECT-DEAD-BRANCH]` 현황을 `PaymentStatusMetricsAspect.java` 재확인 결과(confirm/recovery 매칭 불발, expiration 만 정상, 미매치 시 L93 `unknown` 폴백)로 재기술. `docs/context/CONFIRM-FLOW.md` §5·§6 의 stock-committed 발행 key(orderId→productId)·상수명(`STOCK_COMMITTED`→`EVENTS_STOCK_COMMITTED`)·D5 `markIfAbsent` 시그니처(5-인자→4-인자) 3건을 `PaymentConfirmResultUseCase.java:232-236`/`PaymentEventDedupeStore.java:25` 재확인 후 정정, 문서 내 동일 주장 반복 위치 grep 전수 확인(다른 잔존 0건), 헤더 "최종 갱신" 동기화.
- `./gradlew test` — 문서 전용 변경(코드 무변경), 재실행 대상 아님.
