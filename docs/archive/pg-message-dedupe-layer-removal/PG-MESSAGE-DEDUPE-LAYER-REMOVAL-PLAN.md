...
reviewer·domain-expert 모두 **pass**. critical 0 / major 0 / minor 2. 코드 수정 finding은 없었고, 두 건 모두 영구 문서가 제거된 층을 현재형으로 서술하는 문제라 ship 문서 동기화(B2)에서 처리한다.

| # | 출처 | finding | 처리 |
|---|---|---|---|
| 1 | reviewer | `TODOS.md`의 검토 항목이 "처방 후보 (1)/(2)/(3) 중 판단 전" 미결 상태로 남아 있다. 실제로는 제거안이 채택·실행됐고 판단 근거로 삼자던 로그 종류 자체가 사라졌다 | **채택** — B2에서 항목 제거 (완료 이력은 아카이브 브리핑이 SSOT) |
| 2 | domain-expert | `PITFALLS` / `ARCHITECTURE` / `PAYMENT-FLOW` / `INTEGRATIONS` / `STRUCTURE` / `STACK` / `PAYMENT-FLOW-GUIDE`가 제거된 필터와 "2단 멱등성"을 현재형으로 서술한다. 후속 토픽이 같은 코드 경로를 다시 건드릴 예정이라 착수 전에 정리돼야 한다 | **채택** — B2에서 일괄 갱신. PLAN이 `PAYMENT-FLOW.md`만 지목했는데 실제 대상이 더 넓다는 지적을 반영해 범위를 확장 |

**리뷰에서 확인된 사항** (별도 조치 불필요):

- 제거 완전성 — 포트·어댑터·Fake·로그 종류·설정 키·라이브러리 모두 참조 0건
- 과잉 제거 없음 — payment의 캐시 의존, 인프라 compose의 캐시 서비스 정의, 알람 테스트의 payment 시리즈 2건 모두 보존
- 커밋 분할 안전성 — reviewer가 9개 커밋을 각각 checkout해 빌드. RED 커밋만 의도된 실패, 이후 전부 통과하며 테스트 수(408→408→408→407→407→406)가 PLAN 기재와 일치
- 재수신 테스트 2건이 tautology가 아님 — 실제 인스턴스 스파이 + 접수 기록 수 단언으로 회귀를 잡는 구조
- 되돌리기 보정 제거가 오히려 안전 — 기존 코드는 되돌리기 자체가 실패하면 원 예외를 가릴 수 있었다

**범위 밖으로 남긴 것**: `PaymentConfirmConsumerTest`의 나머지 4개 분기 테스트가 여전히 "벤더 호출 0회"를 단언하는데, 리스너가 벤더를 부를 경로가 없어 항상 참이다. 같은 분기의 라우팅 검증은 `PgConfirmServiceTest`가 실질적으로 담당하고 있어 커버리지 손실은 없다. 후속 정리 대상.
