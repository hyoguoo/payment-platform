...
ship 코드 리뷰 1라운드 — Reviewer revise(major 1 / minor 2), Domain Expert pass(findings 0). 실질 코드 결함 지적은 없었고 세 건 모두 커밋 이력·규약 문제다.

| # | finding | 심각도 | 처리 | 사유 |
|---|---|---|---|---|
| 1 | Task 1·2·6이 `tdd=true`인데 실패 테스트 커밋과 구현 커밋이 하나로 합쳐짐 | major | 스킵 (기록만) | 원인은 오케스트레이터의 dispatch 지시였다 — 세 태스크에 "커밋은 이 태스크 단위로 하나만 만든다"를 넘겨 RED/GREEN 분리 규약과 충돌시켰고, Task 3부터 그 문구를 빼자 정상 분리됐다. 테스트는 모두 존재하고 통과하며 각 태스크 완료 결과에 실패 확인 절차가 기록돼 있다. 커밋 경계만 다르고 기능·검증 영향이 없어 이력 재작성의 실익이 위험보다 작다고 판단 |
| 2 | `eb3d10a2` 커밋 제목에 scope 누락 (pg 단일 서비스 변경인데 `(pg)` 없음) | minor | 스킵 (기록만) | 이미 병합된 이력이고 실질 영향 없음 |
| 3 | 같은 성격(검출 규칙 도입)인 Task 11·12 커밋 type 불일치 (`chore` vs `feat`) | minor | 스킵 (기록만) | Task 12는 커스텀 검사 클래스 신설과 의존 추가를 동반해 `feat`도 무리가 없다. 실질 영향 없음 |

Reviewer가 실측으로 확인한 무결 항목: 루트 프로젝트 검사 비활성화가 서비스별 검사 범위를 줄이지 않음(6개 서비스 재실행 0 위반) / Task 4의 즉시 발행 비활성화가 검증 대상 경로를 우회하지 않음(50회 재실행 통과) / 마스킹 클래스 5벌이 해시 비교로 완전 동일 / 락 계약·추적 구간 테스트가 구조 미러링이 아닌 실제 회귀 검출 형태 / 신규 스타일 위반 0건.

### 리뷰 강도 대조 (설계 "리뷰 강도 대조" 결정 이행)

- [x] 이 토픽의 리뷰 라운드에서 도메인 검토자가 새로 잡아낸 중대 발견이 있었는지, 그것을 일반 검토자가 놓쳤는지 대조
- [x] 대조 결과를 `docs/context/CONCERNS.md` C-11(리뷰 판정 강도 하향)에 기록
- [x] 놓친 중대 발견이 있었다면 C-11의 원복 조건에 따라 검토자 판정 강도를 즉시 되돌림 — 해당 없음

> 대조 결과: ship 코드 리뷰에서 도메인 검토자는 findings 0건으로 통과했다. 일반 검토자가 놓친 도메인 결함을 도메인 검토자가 뒤늦게 잡아낸 사례가 없으므로 C-11의 원복 조건에 해당하지 않는다. 판정 강도 하향을 유지한다.
>
> 다만 설계 단계에서는 양상이 달랐다. discuss 게이트에서 도메인 검토자가 세 라운드에 걸쳐 일반 검토자가 짚지 못한 중대 지적을 냈다 — 격리 대응 근거가 다른 토픽의 도구를 잘못 인용한 점, 충돌 예외를 잡고 같은 영속성 세션을 이어 쓰는 패턴의 위험, 읽기 스냅샷 때문에 확인 조회가 앞선 행을 놓치는 경로. plan 게이트에서도 잠금 읽기가 실행 가능한 형태로 내려오지 않은 것을 잡았다. 이 지적들이 없었다면 구현은 세션 오염과 스냅샷 격리라는 두 결함을 안고 진행됐을 것이다.
>
> 시사점: 강도 하향의 사각은 완성된 코드 리뷰보다 **설계 단계**에서 드러났다. C-11의 원복 판단 기준을 코드 리뷰 대조에만 두면 이 양상을 놓친다. 도메인 인접 토픽에서는 설계 게이트에 도메인 검토자를 반드시 배석시키는 것이 강도 설정보다 실효가 컸다.
