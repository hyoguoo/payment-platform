...
**완료 결과**: `jacocoTestReport` / `jacocoTestCoverageVerification` 블록 + `classDirectories` filter(excludePatterns) 전체를 루트 `build.gradle` `subprojects` 블록으로 이전. `afterEvaluate { def itTask = tasks.findByName('integrationTest') ... }` 조건부 wiring 으로 integrationTest exec 를 합산(payment/pg/product 3서비스 실행, user/gateway/eureka 가드 false 스킵). payment-service `build.gradle` 의 개별 jacoco 블록(jacocoTestReport + jacocoTestCoverageVerification, 구 L121~192) 제거. **`*Application.class` 패턴을 `**/*Application.class` 로 수정**해 서비스별 ApplicationClass 이름(PaymentPlatformApplication 등)에 무관하게 루트 공통 excludes 적용. Gradle 8.10 + Java 24 환경에서 루트 build.gradle 재컴파일 시 `groovyjarjarasm.asm.ClassReader` 가 Java 24 bytecode(class file major version 68)를 지원하지 못하는 문제(Groovy 3.0.22 ASM 제한)로 빌드 실패 → **선행 독립 커밋(`build:`)으로 Gradle 8.14.4(Groovy 3.0.25, ASM 9.7+) wrapper 업그레이드**(C-1 루트 공통화의 전제조건, Java 24 호환, 사용자 승인). jacoco 공통화 커밋과 관심사 분리. **integrationTest exec 합산 전후 application+domain LINE 커버리지**: 합산 전 524/588 = **89.12%** → 합산 후 546/588 = **92.86%** (+3.74pp). 4서비스(payment/pg/product/user) `jacocoTestReport` 오류 없이 실행 확인. `./gradlew :payment-service:test` 416 PASS(단위) + 27 PASS(통합) 회귀 0.
...
**완료 결과**: 서비스별 integrationTest 합산 후 LINE COVEREDRATIO 실측 및 minimum 설정 완료.

| 서비스 | 실측 LINE% (integrationTest 합산) | 안전마진 | minimum |
|---|---|---|---|
| payment-service | 92.87% (547/589) | 3%p | **0.89** |
| pg-service | 94.00% (595/633) | 3%p | **0.91** |
| product-service | 45.65% (21/46) | 5%p | **0.40** |
| user-service | 0.00% (0/3) | — | **0.00** (기본값, 측정 대상 3라인 미커버) |

설계 변경: `element = 'CLASS'` → **`element = 'BUNDLE'`** 로 변경. CLASS element 는 클래스별 minimum 을 강제해 테스트 없는 관리 기능 클래스(AdminPaymentServiceImpl 등)가 0% 이면 전체 플로우가 차단됨 — BUNDLE 단위 합산이 실측 기반 minimum 과 의미적으로 정합.

`jacocoTestCoverageVerification` wiring 개선: `afterEvaluate` 에서 `executionData(test.exec + integrationTest.exec)` + `classDirectories(excludePatterns)` + `violationRules.limits.minimum` 동적 주입. (서비스 ext 는 subprojects 블록 이후에 반영 — afterEvaluate 에서 읽어야 minimum 이 올바르게 적용됨.)

**음성 검증**: payment-service `minimum=1.0` 설정 → `Rule violated for bundle payment-service: lines covered ratio is 0.9, but expected minimum is 1.0` FAIL 재현 → 0.89 로 원복.

**범위 밖 사전 부채 4건 — 별도 `fix:` 커밋으로 분리** (review major F1 반영): 4서비스 전체 `./gradlew build` 가 4서비스 정적분석을 돌리며 드러난 기존 부채(payment 외 pg/product 포함)라 build GREEN 의 전제였으나, C-2(jacoco 게이트) 커밋과 관심사가 달라 독립 `fix:` 커밋으로 분리. 항목: `StockCompensationRecoveryIntegrationTest.java` 미사용 `Objects` import(checkstyle UnusedImports), `PgOutboxImmediateWorkerMdcPropagationTest.java` NP_NULL(ReflectionTestUtils.getField null 가드), `FakePgEventPublisher.java` EI_EXPOSE_REP/EI_EXPOSE_REP2(compact constructor + headers() accessor defensive copy), `FakeEventDedupeStoreTest.java` DLS_DEAD_LOCAL_STORE(dead 변수 제거). 전부 코드 정정(억제 0건).

4서비스 `jacocoTestCoverageVerification` PASS. `finalizedBy jacocoTestCoverageVerification` 연결 확인(jacocoTestReport 후 verification 자동 실행). `./gradlew :payment-service:build :pg-service:build :product-service:build :user-service:build` GREEN. `./gradlew :payment-service:test` 416 PASS + integrationTest 27 PASS 회귀 0.
