...
- [x] Task 4: OutboxWorker 리팩터링 — OutboxProcessingService 위임
- [x] Task 5: OutboxImmediateWorker 구현 (SmartLifecycle + Worker VT)
- [x] Task 6: OutboxImmediateEventHandler 단순화 (@Async 제거, channel.offer)
- [x] Task 7: AsyncConfig 정리 + 환경변수 교체
...
**완료 결과**
> `3.3.3` → `3.4.4` 단일 라인 변경. MySQL Connector/J BOM 위임 유지. 기존 테스트 261개 전체 통과 (breaking change 없음).
...
**완료 결과**
> `core/channel/PaymentConfirmChannel.java` 신규 생성. `LinkedBlockingQueue<String>` 래퍼로 `offer()`/`take()` 제공. `@Value("${outbox.channel.capacity:2000}")` 용량 주입, `@Component` 등록.
...
**완료 결과**
> `OutboxProcessingService.process(String orderId)` 신규 생성. `OutboxWorker.processRecord()` 로직을 그대로 이전. `OutboxProcessingServiceTest` 6개 시나리오 작성 및 통과. `PaymentTossRetryableException`이 checked exception이므로 테스트 메서드에 `throws Exception` 추가 필요.
...
**완료 결과**
> `OutboxWorker`에서 `PaymentCommandUseCase`, `PaymentLoadUseCase`, `PaymentTransactionCoordinator` 직접 의존 제거. `processRecord()` → `outboxProcessingService.process(orderId)` 위임으로 교체. `OutboxWorkerTest`는 배치 흐름(위임 횟수) + 타임아웃 복구 검증만 남김.
...
**완료 결과**
> `OutboxImmediateWorker` 신규 생성 (`SmartLifecycle`). `start()`에서 VT/PT worker N개 기동, `stop(Runnable)`에서 interrupt + join(5s) 후 callback 호출. `workerLoop`에서 `InterruptedException`은 interrupt 복원 후 루프 종료, 그 외 예외는 warn 로그만 찍고 Worker 생존. `SmartLifecycle`은 `stop()`도 abstract이므로 `stop(() -> {})` 위임으로 구현.
...
**완료 결과**
> 의존성 4개 → `PaymentConfirmChannel` 1개로 축소. `@Async` 제거. `handle()`은 `channel.offer(orderId)` 호출 후 false면 warn 로그. `@TransactionalEventListener(phase=AFTER_COMMIT)` 유지.
...
**완료 결과**
> `AsyncConfig.java` 전체 삭제 (`@Async` 사용처 없음 확인). `application-benchmark.yml`의 `outbox.immediate-handler.concurrency-limit` → `outbox.channel.worker-count`, `outbox.channel.capacity`로 교체. `docker-compose.yml`의 `IMMEDIATE_CONCURRENCY_LIMIT` → `WORKER_COUNT`, `CHANNEL_CAPACITY`로 교체. `run-benchmark.sh`의 `concurrency_limit` → `worker_count` 파라미터 교체.
