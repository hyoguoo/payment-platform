...
**반복 3 — 측정 설계의 빈틈: 경사로 평균은 능력이 아니다** — 여기까지의 지점은 전부 `CONSTANT_RATE`(고정 도착률) 모드로 돌렸으므로 k6 자체의 부하 곡선은 경사로가 아니었다. 다만 판정을 부하 종료 후 settle 대기까지 끝낸 **최종 미종결 수**와 **구간 평균 처리율**로만 내려, 부하 도중 적체가 실제로 어떻게 움직였는지(계속 쌓이는지 vs 일정 범위에서 오르내리는지)를 보지 못하는 빈틈이 있었다 — 이 빈틈은 오케스트레이터 지적으로 발견했다. `bench-scaleout-cycle.sh`에 `start_backlog_stat_sampler`/`stop_backlog_stat_sampler`/`backlog_trend_json`을 추가해, 부하 구간 동안만(정산 대기로 들어가기 직전 멈춘다 — 회수·재시도가 적체를 줄이는 구간은 "부하 중 증가하는가"와 다른 질문이다) 미종결(`payment_event.status IN READY/IN_PROGRESS/RETRYING`) 건수를 10초 간격으로 재 결과 JSON `backlog_trend`(시간순 표본 배열 + 최초/최종/최댓값)에 남기도록 했다. 자동으로 "능력 초과"를 판정하지 않고 표본을 사람이 읽어 판단한다(다른 정합 게이트와 같은 철학).

**능력 구간 — 기준 구성(인스턴스 1)에서 60/70/80 req/s 세 점, 각 3분(DURATION=180s) 고정 도착률**

| 지점 | 시작 잔류 | confirm/s | db_done/s | 미종결(정산 후) | 부하 중 적체 추이(10초 간격) | 판정 |
|---|---|---|---|---|---|---|
| r60 | 0 (clean) | 53.13 | 53.13 | 0 (PASS) | 0→172→…→214(峰)→…→151(끝, 감소세) — 초기 스파이크 후 170~214 대역에서 진동, 후반 자연 감소 | **능력 안** |
| r70 | 0 (clean) | 61.67 | 61.37 | 63 | 0→391→220(저점)→…→633(峰, 샘플18)→593(끝) — 저점 이후 15개 표본 동안 거의 단조 증가 | **능력 밖** |
| r80 | 0 (clean) | 70.24 | 67.79 | 514 | 0→491→415(저점)→…→926(峰, 샘플15)→834(끝) — r70 과 같은 모양, 절대량이 더 크다 | **능력 밖** |

r60 은 적체가 특정 대역(170~214)에서 진동하다 부하가 끝나기도 전에 줄어드는 반면, r70/r80 은 저점 이후 부하 구간 대부분(약 150초)에서 거의 단조로 쌓인다 — 성격이 다르다. **단일 인스턴스 지속 가능 능력은 60~70 req/s 사이(70 은 이미 밖)** 로 좁혔다. 이전까지 이 토픽이 "천장 84~88 req/s"로 불러온 값은 과부하 지점에서 쌓인 적체를 다 걷어내는 **배출 속도**였지 지속 가능한 **능력**이 아니었다 — 이번 측정이 그 둘을 갈랐다.

**측정 중 발견 — mysql-payment `max_connections` 인프라 결함, 즉시 수정(Rule 1)** — 판정선 계산을 위해 인스턴스 2대로 처음 돌린 r100 사이클이 정합 검증 단계에서 `DB 접속 실패 — 컨테이너(payment-mysql-payment) 또는 인증 확인`으로 죽었다(exit 1, 캐시는 비우지 않고 중단). 원인을 규명하니 `ERROR 1040: Too many connections` — 실측한 `max_connections`가 300(의도값)이 아니라 MySQL 기본값 151이었다. `docker compose`는 여러 override 파일에 걸친 `command:` 배열을 병합하지 않고 마지막에 스택된 파일이 통째로 덮어쓰는데, `bench-scaleout-cycle.sh`가 `docker-compose.scaleout.yml`을 `docker-compose.benchmark.yml` 뒤에 스택하면서 scaleout.yml 의 mysql-payment `command`(복제 플래그만)가 benchmark.yml 의 `command`(`--max_connections=300` 등)를 조용히 가리고 있었다 — Task 7 이래 이 두 파일을 함께 스택하는 모든 라이브 측정이 실제로는 151 로 돌고 있었다는 뜻이다(인스턴스 1대는 Hikari 풀(기본 80) 하나만 걸어 151을 넘긴 적이 없어 이번에 처음 드러났다). 원장 상태(DONE↔COMMITTED, READY↔NOISE 1:1 대응, 상품/건별 불일치 없음)를 먼저 확인해 데이터 정합 문제가 아님을 확정한 뒤, `docker-compose.scaleout.yml` 의 mysql-payment `command` 에 benchmark.yml 의 세 플래그를 합쳐 넣어 이후 어떤 조합으로 스택해도 max_connections 가 살아있게 고쳤다 — 다음 사이클의 스택 기동이 컨테이너를 자동 재생성해 `max_connections=300`이 실제로 적용됨을 확인했다.

**반복 4 — 그 위에서 판정선을 계산한다(인스턴스 1 vs 2, 능력을 넘는 r100 고정)**

| 구성 | 시작 잔류 | confirm/s | db_done/s | 미종결(정산 후) | 부하 중 적체 최댓값 | 인스턴스 유지 확인 |
|---|---|---|---|---|---|---|
| 인스턴스 1대 | 0 (clean) | 98.03 | 94.87 | 588 | 2329(끝 1719, 감소세) | `wait_all_healthy` 통과 |
| 인스턴스 2대 | 0 (clean) | 78.72 | 75.68 | 641 | **3648**(끝 921, 급감) | `payment-service 2대 healthy + Eureka 등록 확인` 로그 + `dc ps` 로 `docker-payment-service-1/2` 두 컨테이너 확인, consumer group 도 두 client-id 로 분할 확인 |

**판정선 — 확정 처리율(db_done/s) 비 = 75.68 / 94.87 = 0.80x.** (confirm/s 비로도 78.72/98.03 = 0.80x, 같은 값) 판정 기준(1.6배)에 한참 못 미친다 — 그냥 "안 먹힌다"가 아니라 **2대가 1대보다 더 나빴다**(처리율도 낮고 적체 최댓값도 1.6배 더 컸다). 인스턴스 대수를 확인한 상태에서, 능력을 확실히 넘는 부하로 잰 유일한 유효 판정선이다.

---

### Task 21 최종 정리 — 배제된 후보 전체 목록과 결론

**배제된 후보(이 토픽 전체, Task 20부터 누적)**
1. 고정 동시성 상한(`ConfirmedEventConsumer` concurrency, pg-service inbox/outbox worker 수) — 3~4배로 올려도 능력 안 늘어남(1차 반복)
2. Kafka EOS 트랜잭션 직렬화(메시지당 4단계 브로커 왕복) — 꺼도 능력 안 늘어남(판별 사이클)
3. 발행 경로의 다른 고정 상한 — `OutboxRelayService`가 쓰는 `@Async("outboxRelayExecutor")`는 상한 없는 가상 스레드(`AsyncConfig`), 고정 주기 `OutboxWorker`(2초/배치 100)는 폴백일 뿐 — 오케스트레이터가 코드로 확인, 고정 상한 후보 아님
4. MySQL 행 잠금 대기 — 전 구간 0
5. Redis(재고 캐시·멱등 저장소) 블록된 클라이언트 — 전 구간 0
6. 복제 지연 — 평균 0.1초 이하
7. k6 자체 CPU(부하 도구 오염) — 평균 6~15%
8. Kafka 브로커 CPU·디스크 IO — 최대 1.3코어, 여유
9. 호스트/컨테이너 CPU — 전 컨테이너 평균 합이 10코어 중 1.2~1.5개뿐이라 포화 근거 없음(다만 확정 배제는 아니다 — "그 총량에 닿는 관측"이 없을 뿐)

**최종 결론**
- **단일 인스턴스 지속 가능 능력은 60~70 req/s 사이** — 이전 리포트가 인용한 84~88 req/s 는 과부하 상태에서의 배출 속도였다.
- **판정선(인스턴스 1→2 확정 처리율 1.6배)은 능력을 넘는 부하에서 0.80x 로 미달, 그것도 역방향(2대가 더 나쁘다).** 스케일아웃이 이 환경에서 먹히지 않는다는 것을 이 토픽에서 유일하게 유효한 조건으로 확정했다.
- 지목 가능한 설정 손잡이(동시성·워커 수·EOS·발행 실행기)를 전부 늘리거나 꺼봤지만 능력이 안 늘었고, 인스턴스를 늘리는 것도 역효과였다. CPU·잠금·블록·복제 지연·Kafka 브로커는 전부 여유로워 자원 포화로 설명되지 않는다 — 원인은 이 토픽 범위(설정값 손잡이 재측정)에서 확정하지 못한 채 남긴다. 2대가 1대보다 나쁘다는 결과는 늘어난 병렬성이 새 용량을 못 얻고 같은 공유 자원(단일 Kafka 브로커, 단일 MySQL 원본, 인스턴스 간 조정 오버헤드)을 더 잘게 나눠 쓰며 오버헤드만 늘어난다는 그림과 일관된다 — 다음은 요청당 처리 경로 프로파일링이나 공유 자원 자체의 확장(다중 브로커/파티션 재설계 등)이지 이 토픽의 설정값 조정이 아니다.

**정리** — 능력 구간 3점(r60/70/80) + 판정선 2점(i1/i2 r100) 전부 부하 구간 시작 시점 잔류 0(clean)에서 시작했다. r60 만 PASS(자동 재구성), 나머지 네 점은 INCONCLUSIVE 로 끝나 캐시를 자동으로 비우지 않는다 — 매 지점 Kafka consumer lag 을 확인한 뒤(이번 라운드는 인스턴스별 소비 스레드가 느려 자연 배수까지 기다리는 대신, 컨슈머를 먼저 멈추고 `--reset-offsets --to-latest` 로 밀린 오프셋을 건너뛴 뒤 비우는 절차 — 스크립트가 문서화한 대안 경로 — 를 반복 사용해 대기 시간을 줄였다) 사람이 직접 payment 원장 여섯 테이블을 TRUNCATE, 재고를 재시드해 다음 지점이 깨끗한 상태에서 시작하도록 복원했다. 2인스턴스 사이클의 접속 실패(정합 불일치 아님, 인프라 결함으로 확인 후 위에서 수정)도 원장 상태를 먼저 확인한 뒤에만 비웠다. payment-service/pg-service 는 정지 상태로 남아 있다(다음 사이클이 재기동한다). 결과 파일은 `results/task21-cap-r{60,70,80}-cycle.json`, `results/task21-scale-i{1,2}-r100-cycle.json`(+ 각 `-verdict.json`/`.json`)로 보존. 커밋에는 backlog 표본화 스크립트 변경과 `docker-compose.scaleout.yml` max_connections 수정만 남는다.

---

### Task 21 최종 측정 (예열·동일 설정에서 재측정)

앞선 판정(0.80배)은 오염이라 폐기한다 — 1대는 커넥션 상한 151 시절에, 2대는 300으로 고친 뒤 MySQL 재기동 52초 만에 쟀다(버퍼 풀 비어 있음). 같은 설정·예열 상태에서 도착률 180으로 다시 쟀다.

| 구성 | 접수/s | 종결/s | 적체 최대 | MySQL 실행 스레드 | 행 잠금 대기 |
|:---:|---:|---:|---:|---:|---:|
| 인스턴스 1 | 153.0 | 149.2 | 21,886 | 13 | 0 |
| 인스턴스 2 | 156.0 | 100.3 | 27,485 | 35 | 0 |

**판정선 — 1 → 2 종결 처리율 0.67배. 목표 1.6배 미달이며, 인스턴스를 늘리면 오히려 나빠진다.**

**능력 수치 정정** — 인스턴스 1대가 149/s 를 종결시킨다. 앞서 적은 "능력 60~70/s"와 "천장 85/s"는 둘 다 틀렸다. 전자는 처리 중 물량(WIP)이 느는 것을 못 받아내는 것으로 오독한 결과이고, 후자는 경사로 부하의 창 평균이었다.

**2대에서 나빠지는 정황** — MySQL 실행 스레드가 13에서 35로 늘지만 행 잠금 대기는 0이다. 확정 결과 토픽 파티션이 3개라 2대에 2:1 로 갈리는 편향(`TODOS.md` 기등재)도 후보다. 원인은 이 태스크에서 규명하지 않았다.

**이 측정에서 반복된 오독 유형** — 무엇을 재고 있는지 착각한 것이 세 번이다. (1) 부하 도구 한계를 시스템 한계로, (2) 경사로 창 평균을 능력으로, (3) 처리 중 물량을 못 받아낸 대기열로. 셋 다 수치는 맞고 해석이 틀렸다. 다음 측정에서 같은 함정을 피하려면 매번 "이 값이 무엇을 세고 있나"를 먼저 확인해야 한다.
...
리뷰 실행: 2026-09-01, reviewer + domain-expert 병렬. **critical 0 / major 0 / minor 1.**

|finding|판정|사유|
|:---:|:---:|:---:|
|minor — 복제본을 켜면 `GET /status` 폴링이 복제 지연 창에서 404 를 반환할 수 있다|**의도적 스킵**|복제본 라우팅은 기본값이 꺼져 있어 벤치마크 조건에서만 열리는 창이고, 돈 경로 판정 일곱은 그대로 원본을 읽어 영향이 없다. 상시 운영 승격 시 확인할 항목으로 `TODOS.md` 에 남겼다|

리뷰 중 함께 확인한 사실 — **파티션 수 3 이 네 곳에 하드코딩돼 있다** (`create-topics.sh`, 양 서비스 `KafkaTopicConfig`, 브로커 기본값).
하나만 바꾸면 서로 어긋나므로 단일 출처로 모으는 것이 복제본 상시 운영 승격의 선행 작업이다. `TODOS.md` 등재.

리뷰 이후 코드 변경 없음 — 후속 커밋은 전부 문서다. 재리뷰 생략.
