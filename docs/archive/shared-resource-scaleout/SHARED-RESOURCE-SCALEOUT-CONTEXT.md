...
- **사이클 사이 재구성은 다섯 단계** — 부하 정지와 배출 확인, 미종결 0 안정 확인, 소비 적체 안정 확인, 회수 주기 정지, 그리고 두 확인을 다시 통과한 직후 비우고 재시드. 확인과 비우기 사이가 열려 있으면 낙오 확정의 캐시 차감분이 지워져 같은 재고 단위가 다음 사이클에서 또 팔린다 — 원본은 양수라 음수 가드에도 안 걸리는 조용한 초과 판매다.
...
| 결제는 종결됐는데 상품 서비스가 재고 확정을 아직 소비 중이다 | 재시드가 원본과 캐시를 같은 상수로 덮은 뒤 뒤늦은 소비가 도착하면 원본만 그만큼 깎여 캐시가 원본보다 커진다. 다음 사이클이 그 차이만큼 더 통과시키고, 초과분은 벤더 승인까지 난 뒤 재고 확정 음수 가드에 걸려 격리되므로 사람이 환불을 판단해야 하는 사고가 된다. 재시드 전에 소비 적체가 안정됐는지 확인하고, 정합 판정 자체는 상품별 캐시-원본 대조에 맡긴다 — 소비가 안 끝났으면 원본이 아직 안 깎여 그 대조에서 불일치로 잡힌다 |
...
## 실측 정정 (2026-08-26, execute 중)

**소비 적체 0 은 도달할 수 없는 기준이었다.** 재고 확정 발행이 트랜잭션으로 묶여 있어 커밋 표시가 파티션마다 오프셋을 하나씩 차지하는데, 소비자는 그것을 레코드로 처리하지 않는다. 그래서 실거래가 있던 뒤에는 소비자가 정상으로 붙어 있어도 파티션당 1이 남고, 기다려도 줄지 않는다. 오프셋을 강제로 최신으로 옮겨야만 0 이 되는데, 그 조작은 진짜 미소비 메시지까지 건너뛰게 만들어 정합 검증의 의미를 없앤다.

기준을 바꿨다 — 적체 게이트는 **파티션 수 이하이면서 더 줄지 않고 소비자가 붙어 있는 상태**를 본다. 정합 판정의 권한자는 적체 지표가 아니라 **상품별 캐시-원본 대조**다. 소비가 안 끝난 채로 재면 원본이 아직 안 깎여 캐시가 원본보다 커지고, 그 차이가 상품별 대조에서 그대로 불일치로 잡힌다. 설계가 지키려던 안전 성질(소비가 남은 채로 재시드하지 않는다)은 그대로 유지된다.

소비자가 아예 안 붙어 있으면 적체는 영원히 안 줄므로, 두 스크립트 모두 소비자 생존을 먼저 확인하고 없으면 기다리지 않고 실패한다.
