...
flowchart TD
    ...
    subgraph PGQ["pg-service 이력 조립"]
        ...
        Q4["회차별 타임라인 조립<br/>회차는 행 헤더에서 읽는다"] --> Q5{"결제가 종결됐는가"}
        Q5 -->|"아니오"| Q7["정상 시도<br/>비교 대상 없어 판정 스킵"]
        Q5 -->|"예"| Q9{"발행 시각이 종결보다 늦은가<br/>미발행이면 예정 시각으로"}
        Q9 -->|"예"| Q6["예약됐으나 미실행<br/>좀비 회수가 앞질렀다"]
        Q9 -->|"아니오"| Q7
...
| 종결 이후 발행 행의 취급 | `pg_inbox` 의 최종 상태와 종결 시각을 1차 기준으로 삼고, **발행 시각(없으면 실행 예정 시각)** 이 종결 시각보다 늦은 행은 **예약됐으나 미실행**으로 따로 라벨링한다. 결제가 비종결이면 비교 대상이 없어 판정을 건너뛴다 | 좀비 회수가 예약된 재시도를 앞지르면 벤더 호출 없는 발행 행이 남는다. 이를 정상 시도로 세면 화면이 "종결 이후에 시도가 더 있었다"는 거짓을 시간 역전 순서로 보여주고, 격리 건 판단이 그 거짓 위에서 내려진다 |
