...
| **변경 패턴** | `redisTemplate.getConnectionFactory().getConnection()...` 체인 → `RedisConnectionFactory factory = Objects.requireNonNull(redisTemplate.getConnectionFactory(), "RedisConnectionFactory must not be null"); factory.getConnection()...` (try 블록 내 외부 변수 재할당 금지 준수) |
| **완료 기준** | `./gradlew :payment-service:spotbugsTest` 에서 NP_NULL 위반 4건 사라짐. `./gradlew :payment-service:test` 회귀 0 (기존 358 PASS 유지). |
| **매핑 결정** | D-SB1 |
| **선행 태스크** | 없음 |
