...
- 주요 변경 파일:
  - `build.gradle`
  - `core/config/AsyncConfig.java`, `core/config/PaymentConfirmChannel.java` (신규)
...
**구현**
- `src/main/java/com/hyoguoo/paymentplatform/core/config/PaymentConfirmChannel.java` (신규)
...
- `src/test/java/.../payment/scheduler/OutboxImmediateWorkerTest.java` (신규)
  ...
  - `stop_호출후_Worker가중단된다`:
    - `worker.start()` → `worker.stop()` → interrupt 후 추가 처리 없음 검증
- 패턴: Awaitility (`await().atMost(1, SECONDS).untilAsserted(...)`)
- `testImplementation 'org.awaitility:awaitility:4.2.2'` — `build.gradle` 의존성 추가
...
- `src/main/java/.../payment/scheduler/OutboxImmediateWorker.java` (신규, `@Component`, `SmartLifecycle`)
  ...
  - `start()`: Worker 스레드 N개 생성 (`Thread.ofVirtual()` 또는 `Thread.ofPlatform()`)
    - 각 Worker: `while (!Thread.currentThread().isInterrupted()) { orderId = channel.take(); outboxProcessingService.process(orderId); }`
    - `take()`에서 `InterruptedException` → interrupt 복구 후 루프 종료
  - `stop(Runnable callback)`: 모든 Worker `interrupt()` → `callback.run()`
  - `isRunning()`: 상태 플래그 반환
...
- `OutboxImmediateEventHandlerTest.java` 대폭 수정:
  ...
  - `@TransactionalEventListener`, `@Async` 애노테이션 선언 검증 테스트 추가 (리플렉션 방식)
