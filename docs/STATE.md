...
> **다음 세션 진입점**: TIME-MODEL-FOLLOWUP execute 단계, **Task P2**(JdbcEventDedupeStore — NOW() 제거 + existsValid 제거 + recordIfAbsent now 바인딩)부터. PLAN: `docs/TIME-MODEL-FOLLOWUP-PLAN.md`.

## 활성 작업

- **TIME-MODEL-FOLLOWUP** (stage: **execute**, 활성 태스크 **P2**, 이슈/브랜치 #89) — TIME-MODEL-AND-EXPIRY(#83) 이연 후속 3건 한 PR 묶음
