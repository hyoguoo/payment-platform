...
> 최종 수정: 2026-05-11 — CBA-3+CBA-5 완료 (user-service Flyway db/schema+db/seed 분리 + yml locations override). 활성 태스크 CBA-7.

## 활성 작업

- **주제**: CLEANUP-BATCH-A (코드 청소 4건 묶음 — TC-16 + TC-10 + TC-2 + TC-5)
- **단계**: execute (활성 태스크 CBA-7 — FlywayDockerProfileTest — Testcontainers docker profile seed 차단 검증)
