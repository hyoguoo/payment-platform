...
- **단계**: execute (plan 완료)
- **활성 태스크**: Task 11 — 회수 주기 작업 (Task 1~10 완료. Task 10 은 `StockHoldRecordRepository.findNoiseCandidates(limit)` 신규(NOISE 상태만 id 오름차순 최대 limit건, 신규 record `StockHoldRecordCandidate`)와, `StockHoldReverter.revertProductHold` public 승격(9a/9b/9c 와 되돌리기 로직 공유)을 바탕으로 `application/usecase/StockHoldRecoveryUseCase` 를 신설했다. `recover(batchSize)` 가 후보마다 `PaymentEventRepository.findByOrderId` 로 원본 결제 상태를 조회해 종결(`PaymentEventStatus.isTerminal()`)일 때만 되돌리고, 고아 기록(결제를 못 찾음)은 다음 주기로 미룬다. 결제 상태 조회가 쓰기 경로와 같은 한정자 없는 `PaymentEventRepository` 빈을 쓴다는 것을 `StockHoldRecoveryUseCaseOriginReadIntegrationTest`(`@DataJpaTest`, 빈 인스턴스 `isSameAs` 리플렉션 비교)로 구조 고정 — 트랜잭션 readOnly 플래그 기반 판별은 중첩 트랜잭션 참여 시맨틱 때문에 판별력이 없어 검토 중 기각하고 빈 동일성 비교로 대체했다. `./gradlew :payment-service:test` 669건 전체 pass(신규 16건). 주기 작업 배선(스케줄러·배치 크기 설정·지표)은 Task 11 범위)
- **이슈·브랜치**: #144
