...
> 최종 수정: 2026-05-11 — CBA-2+CBA-4 완료 (product-service Flyway db/schema+db/seed 분리 + yml locations override). 활성 태스크 CBA-3.

## 활성 작업

- **주제**: CLEANUP-BATCH-A (코드 청소 4건 묶음 — TC-16 + TC-10 + TC-2 + TC-5)
- **단계**: execute (활성 태스크 CBA-3 — user-service SQL 파일 이동 (migration → schema/seed))
