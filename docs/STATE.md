...
> **다음 진입점**: T1~T11, T8 완료(payment/pg 시간 추상화 + 만료 정책). pg 단위 311/311 PASS. 다음 활성 태스크 T15(pg 벤더 strategy 시각 정리). 이후 T9(스케줄러 키) → T12(dedupe UTC + AC1) → T13(product) → T14.
...
- **TIME-MODEL-AND-EXPIRY** (PR B — 시간 모델 Clock/Instant 통일 + 결제 만료 정책 명문화)
  ...
  - 완료: T1~T11, T8 (도메인/엔티티/저장소 Instant + UTC 저장 일관 + application/infra Clock 전환 + 만료 임계 외부화 + 만료 정책 명문화 + pg 도메인 Instant.now() 제거). `LocalDateTimeProvider` 잔여는 포트 정의 + JdbcPaymentEventDedupeStore(T12 대상)뿐.
  - 활성 태스크: **T15** (pg 벤더 strategy 시각 정리 — TossPaymentGatewayStrategy.LocalDateTime.now() 제거 + approvedAtRaw contract 확인)
  - 이슈 #83, 브랜치 #83
