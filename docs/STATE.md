...
- **단계**: execute (plan 완료)
- **활성 태스크**: Task 10 — 미회수 선차감 회수 판정 (Task 1~9d 완료. Task 9d 는 `StockCachePort` 의 주문 단위 메서드 3종(`decrementAtomic`/`compensateAtomic`/`compensateIfDecremented` 의 `List<PaymentOrder>` 오버로드)과 어댑터의 내부 반복 브리지, `FakeStockCachePort` 의 대응 메서드를 걷었다. `compensateAtomic` 은 9a/9b/9c 가 조건부 되돌리기로 대체하도록 예정돼 있어 애초에 상품 단위로 승격되지 않았던 경로라 그 결과 enum `StockCompensationAtomicResult` 도 파일째 삭제. 어댑터 레벨 테스트 중 다중 상품 브리지 계약은 이제 `PaymentTransactionCoordinatorTest` 가 담당해 정리했고, 유일하게 어댑터에서만 검증되던 두 이례 시나리오(ALREADY_DONE, decrement 부재+compensation 존재)는 상품 단위 호출로 옮겨 보존했다. `PaymentConfirmResultUseCaseIdempotencyGuardTest`/`PaymentConfirmResultUseCaseTest`/`QuarantineCompensationHandler` 에 남아있던 `compensateAtomic` 표기도 정정. `./gradlew :payment-service:test` 653건 + `:payment-service:integrationTest` 151건 전체 pass. 범위 밖 관찰: 캐시 되돌리기 로그(`logCompensation`)가 이번 정리로 사라졌고, `stock_compensation_atomic.lua`/`StockCompensationAtomicLuaTest` 는 Java 호출부는 없어졌지만 스크립트 자산 자체는 이번 태스크 지시 밖이라 그대로 뒀다 — 후속 판단 필요)
- **이슈·브랜치**: #144
