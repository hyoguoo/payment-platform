...
> 최종 수정: 2026-05-09 — PCS-8 완료 (PgInboxProcessor 신규, processPending/processInProgressZombie 두 진입점, 활성 태스크 PCS-9)

## 활성 작업
- **주제**: PG-CONFIRM-LISTENER-SPLIT (pg-service vendor 호출 listener thread 분리, pg-confirm-flow.md 위키 정합)
- **단계**: execute (활성 태스크 PCS-9 — `PgConfirmService` 분기 재배치 + `DuplicateApprovalHandler` + `PgInboxAmountService` 갱신)
