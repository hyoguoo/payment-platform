...
- 단계: **execute**
- 활성 태스크: Task 8 (재고 캐시·멱등 저장소 클러스터 인프라)
- 이슈 / 브랜치: #146
...
- Task 6 이 `RedisConfig`/`StockRedisConfig` 를 노드 목록 프로퍼티로 단독/클러스터 분기하도록 바꿨다 — 노드 목록 미설정(현재 기본값) 상태로 `test` 682건·`integrationTest --rerun-tasks` 670건 전체 통과 확인 완료. "노드 목록을 넣으면 클러스터로 뜬다"는 Task 8 인프라 기동 후 Task 12 에서 실측 예정
...
- **인프라 기동 상태** — 검증에 쓴 스택(mysql-payment/replica, eureka, kafka, redis-dedupe/stock, payment-service 앱 컨테이너 1개)이 내려가지 않고 떠 있다. Task 8 이 이어서 재고 캐시·멱등 저장소 클러스터 컨테이너를 같은 스택에 추가한다
