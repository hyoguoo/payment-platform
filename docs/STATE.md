...
### plan 게이트에서 발견 — 기존 재수신 테스트가 헛돌고 있었다

`PgConfirmService` 가 벤더 호출 서비스를 필드로만 갖고 어느 분기에서도 쓰지 않는다(리스너는 접수·적재까지, 벤더는 워커가 호출). 그래서 기존 재수신 테스트 2건의 "벤더 호출 0회" 단언은 항상 참인 값을 검사하고 있었다. Task 1 에서 실제 흡수를 관측하는 형태로 다시 짜고, 죽은 필드 자체는 Task 6 에서 제거한다.
