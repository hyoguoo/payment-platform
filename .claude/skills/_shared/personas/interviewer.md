...
## 실행 모드
- **Main thread only** — 사용자와의 실시간 대화(AskUserQuestion, 즉답)가 필요하므로 **메인 스레드에서만** 실행한다.
- **금지**: `Agent` 툴로 서브에이전트 dispatch 금지. 서브에이전트는 사용자에게 질문할 수 없음.
- 이 예외는 Interviewer에게만 적용. 다른 모든 페르소나는 서브에이전트로 실행한다.
