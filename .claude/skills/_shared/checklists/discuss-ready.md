...
- [ ] 런타임 행동이 바뀌는 토픽이면 라이브 검증 필요 여부(대상 `docs/smoke/` 가이드, 스택 기동 요구)가 결정됨
...
> 도메인 비접촉 토픽 — 소스·런타임 설정 변경 계획이 없고, 산출물이 결제 도메인 동작을 서술·정정하지도 않는 경우 — 은 이 섹션 전체 n/a, discuss 게이트에서 domain-expert 생략 가능 (`workflow-discuss` 5 참조).
