...
for filename in files:
    ...
    confirm_med  = val('confirm_ms', 'med')
    confirm_p95  = val('confirm_ms', 'p(95)')
    ...
    rows.append((case, tps, confirm_med, confirm_p95, e2e_med, e2e_p95, err_rate, dropped))

header = f"{'케이스':<32} {'TPS':>7}  {'Confirm med':>11}  {'Confirm p95':>11}  {'E2E med':>8}  {'E2E p95':>8}  {'에러율':>7}  {'Dropped':>8}"
sep    = '-' * len(header)
...
for case, tps, confirm_med, confirm_p95, e2e_med, e2e_p95, err_rate, dropped in rows:
    confirm_med_str = f"{confirm_med:,.0f}ms" if confirm_med else "  -"
    confirm_p95_str = f"{confirm_p95:,.0f}ms" if confirm_p95 else "  -"
    e2e_med_str     = f"{e2e_med:,.0f}ms"     if e2e_med     else "  -"
    e2e_p95_str     = f"{e2e_p95:,.0f}ms"     if e2e_p95     else "  -"
    print(
        f"{case:<32} {tps:>6.1f}/s"
        f"  {confirm_med_str:>11}"
        f"  {confirm_p95_str:>11}"
        f"  {e2e_med_str:>8}"
        f"  {e2e_p95_str:>8}"
        f"  {err_rate:>6.2f}%"
        f"  {dropped:>8,}"
