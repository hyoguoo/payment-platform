...
- 플로우차트는 **이해에 필요한 분기·예외를 빠짐없이** 담는다. 노드 라벨은 메서드명 대신 도메인 용어, 코드 식별자는 `()` 부가 표기.
- **도메인 예외**: 다음 두 갈래 중 하나라도 해당하면 상태 전이·예외 분기는 전체 경로를 유지한다(생략 판단을 허용하지 않는다) — 브리핑은 게이트 이전 단계라 체크리스트가 누락을 잡지 못하기 때문이다.
  - 도메인 검토자 포함 대상 토픽 — `discuss-ready.md`의 domain risk 섹션 2갈래 조건으로 판정
  - `domain_risk=true` 태스크를 가진 토픽 — `workflow-plan` 스킬의 분해 원칙에 있는 정의(결제 상태 전이·멱등성·정합성/트랜잭션 경계·PII·외부 PG 연동·race window)로 판정
  - 위 두 조건은 discuss 시점(PLAN.md가 아직 없어 태스크 플래그가 존재하지 않음)과 plan 이후 시점을 각각 커버한다
