...
불명확하면 `AskUserQuestion` 도구로 제안하고 확인받는다.
...
사용자에게 질문할 때는 항상 `AskUserQuestion` 도구를 사용한다.
...
- [ ] GitHub 이슈 생성 (`mcp__github__issue_write` 사용, `gh` CLI 불가)
  - 이슈 제목/본문: 문제 배경 + 구현 방식 + 영향 범위 요약
- [ ] 브랜치 생성 및 전환: `git checkout -b "#<이슈-번호>"` (따옴표 필수)
- [ ] STATE.md 단계를 `plan`, 이슈 번호와 브랜치를 기록
- [ ] `docs/topics/<TOPIC>.md` + `docs/STATE.md` 를 하나의 `docs:` 커밋으로 묶기 (**feature 브랜치에서**)

완료 후: "discuss 완료. 이슈 #<번호> 생성, 브랜치 #<번호>로 전환했습니다. plan 단계로 넘어가겠습니다." 라고 알린다.
