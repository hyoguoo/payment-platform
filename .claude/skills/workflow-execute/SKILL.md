...
- 메인은 반환의 `결과` 필드(implementer 반환 템플릿 첫 줄)를 확인한다 — **성공일 때만** 다음 태스크로 진행한다.
...
- **연속 진행 종료 조건**: `결과: 실패` 또는 `에스컬레이션` 반환 시 연속 dispatch를 즉시 중단하고, 상태 요약(완료 태스크 / 실패 태스크와 사유 / 남은 태스크)을 제시한 뒤 사용자 확인을 기다린다. 실패한 태스크를 건너뛰고 다음으로 넘어가지 않는다 — 뒤 태스크가 앞 산출물에 의존한다.
