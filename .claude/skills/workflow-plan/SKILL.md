...
plan 라운드가 pass하면 **PLAN.md 상단에 `## 요약 브리핑` 섹션**으로 직접 작성한다. 채팅에는 "요약 브리핑을 PLAN.md 상단에 추가했습니다. 확인 후 진행/정정 알려주세요." 한 줄만. 내용:

1. **Task 목록** — 번호 + 한 줄 설명 (도메인 용어)
2. **변경 후 전체 플로우차트** — Mermaid. **간략화 금지, 전체 경로**(모든 분기/예외/상태 전이)를 표현한다. 분기가 많으면 여러 개로 나눠 그린다. **노드 라벨은 도메인 용어 우선** — 메서드명·클래스명·enum 식별자 대신 한국어 도메인 표현. 코드 식별자가 필요하면 `()` 괄호로 부가 표기. 금지 문자: `{` `}` `·` `→` — `writing.md` 참조.
...
- [ ] PLAN.md 상단에 요약 브리핑이 정착되어 있음을 확인 (브리핑 단계에서 이미 작성됨)
