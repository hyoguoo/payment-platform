...
| `CONVENTIONS.md` (인덱스) + `conventions/` 하위 | code-style(Lombok/Naming/주석/안티패턴) · error-logging(예외/LogFmt/AOP) · transactions · kafka · testing | 실제 코드 관례 |
...
### 5. 완료 항목 정리 (TODOS·CONCERNS 등 대장 문서)

완료된 항목은 ✅ 마킹으로 쌓아두지 않고 3분류로 삭제한다.

- (a) **전체 삭제** — 해소 마킹 + archive briefing 경로가 있는 항목
- (b) **혼합 항목은 해소분 문장만 제거** — 완료된 부분만 지우고 잔여 한계·기각 근거는 보존
- (c) **보존** — 수용된 한계, "회피된 우려" 표는 삭제 대상이 아니다

갱신한 문서의 헤더 "최종 갱신" 시점은 본문 최신 내용과 항상 동기화한다.

사실 서술의 근거는 소스 파일:라인만 인정한다. 다른 문서나 archive briefing 을 인용해 사실을 확정하지 않는다 — 문서끼리 서로 인용하면 stale 서술이 정본으로 굳어질 위험이 있다.
