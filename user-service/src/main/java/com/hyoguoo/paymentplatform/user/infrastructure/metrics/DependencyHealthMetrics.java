package com.hyoguoo.paymentplatform.user.infrastructure.metrics;
 ...
 * </ul>
 *
 * <p>DB 조회는 {@code metrics.user.dependency.timeout-seconds} 타임아웃 가드로 감싸
 * Hikari connectionTimeout 등 블로킹 경로로 폴러가 직렬화되는 것을 방지한다.
...
public class DependencyHealthMetrics {
    ...
    public DependencyHealthMetrics(
            ...
            Clock clock,
            @Value("${metrics.user.dependency.timeout-seconds:2}") long timeoutSeconds) {
        ...
        LogFmt.info(log, LogDomain.USER, EventType.METRICS_INIT,
     ...
     */
    @Scheduled(fixedDelayString = "${metrics.user.dependency.polling-interval-seconds:10}000")
    public void poll() {
        dbGauge.set(checkWithTimeout(this::checkDbHealth));
        lastPollTimestamp.set(clock.instant().getEpochSecond());

        LogFmt.debug(log, LogDomain.USER, EventType.METRICS_GAUGE_UPDATED,
                () -> "db=" + dbGauge.get());
    }
    ...
    private long evaluateFuture(Future<Boolean> future) {
        try {
            ...
            future.cancel(true);
            LogFmt.warn(log, LogDomain.USER, EventType.METRICS_GAUGE_UPDATED,
