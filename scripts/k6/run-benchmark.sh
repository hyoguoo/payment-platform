...
echo "  ★ Async Worker: 케이스별 VT/PT 선택 (outbox.channel.virtual-threads)"
echo ""
echo "  부하 단계 (고/저지연 동일):"
...
switch_strategy() {
  ...
  local use_vt=${4:-true}
  local use_parallel=${5:-true}

  print_info "전략: ${strategy} | Toss 지연: ${toss_min_delay}~${toss_max_delay}ms | VT: ${use_vt} | Parallel: ${use_parallel}"

  # 확실한 재기동을 위해 stop 후 up
  ...
  CHANNEL_VIRTUAL_THREADS="${use_vt}" \
  SCHEDULER_PARALLEL_ENABLED="${use_parallel}" \
...
run_sync() {
  ...
  switch_strategy "sync" "${toss_min}" "${toss_max}" "false" "false"
  reset_data
...
run_outbox() {
  ...
  local toss_max=$3
  local use_vt=$4
  local use_parallel=$5
  echo ""
  print_section "--- ${testid} ---"
  switch_strategy "outbox" "${toss_min}" "${toss_max}" "${use_vt}" "${use_parallel}"
  reset_data
...
run_outbox "outbox-high"           "2000" "3500" "false" "false"
run_outbox "outbox-parallel-high"  "2000" "3500" "true"  "true"

# ── 2. 저지연 (100~300ms) — 참조 비교 ──
print_section "=== 2. 저지연 환경 (100~300ms) ==="
run_sync   "sync-low"              "100" "300"
run_outbox "outbox-low"            "100" "300"   "false" "false"
run_outbox "outbox-parallel-low"   "100" "300"   "true"  "true"
...
echo "  항목              sync-high        outbox-parallel-high"
