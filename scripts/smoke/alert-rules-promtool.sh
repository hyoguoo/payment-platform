...
#   라이브 스택 없이 Docker 만으로 실행 가능. 총 14 케이스.
#
# ── 검증 픽스처 ────────────────────────────────────────────────────────────
#   코디네이터 정체 (5 케이스): observability/prometheus/rules/tests/coordinator_test.yml
...
#
#   DLQ 적체 (6 케이스): observability/prometheus/rules/tests/dlq_test.yml
...
print_section "════════════════════════════════════════════════════════════"
print_section "  alert-rules-promtool — 알람 규칙 3그룹 발화 단정 (14 케이스)"
print_section "════════════════════════════════════════════════════════════"
...
run_test "코디네이터 정체 (5 케이스)" "coordinator_test.yml"
run_test "종결 가드 skip (3 케이스)" "guard_skip_test.yml"
run_test "DLQ 적체 (6 케이스)" "dlq_test.yml"

# ── 종합 결과 ───────────────────────────────────────────────────────────────
print_section "════════════════════════════════════════════════════════════"
if [ "${FAIL_COUNT}" -eq 0 ]; then
    print_info "✅ 알람 규칙 promtool test 전체 PASS (3 그룹, 14 케이스)"
    exit 0
else
